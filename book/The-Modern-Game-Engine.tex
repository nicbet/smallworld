...
Have you ever played a console port of a video game on PC and felt the
mouse move like it was swimming through syrup? \emph{Elden Ring}'s PC
launch in 2022 is the textbook example: the game was a masterpiece of
open-world design sitting on top of a deeply uncomfortable
keyboard-and-mouse experience. Mouse movement felt sluggish and
disconnected because the input layer was filtering it through a system
tuned for analog sticks, applying smoothing and acceleration curves that
made sense on a gamepad but felt wrong with a mouse. On-screen prompts
displayed Xbox button icons regardless of whether the player was using a
controller. Key rebinding was limited. The community response was
immediate: mods to fix mouse input appeared within days, and
FromSoftware spent months patching improvements. It's not that Elden
Ring's input system didn't work - it simply had not been designed for
the platform it shipped on. \emph{Elden Ring} was far from alone.
\emph{Dark Souls}' original PC port had similar issues a decade earlier.
\emph{No Man's Sky} launched with restricted rebinding. \emph{Cyberpunk
2077} exposed input latency on PC that console players never noticed.
These are not obscure edge cases - they are the predictable consequences
of an input system that was built for one device and one context, then
asked to serve many.

Every engine reads keyboard, mouse and controller inputs and tutorials
cover it in a page. An modern game engine lets the player rebind keys,
swap between keyboard and gamepad mid-session, open a menu without
accidentally firing a weapon, record input for replay, and present a
consistent experience on platforms with different conventions, all
without exposing the windowing library's types to gameplay code. The
naive approach, where game systems read raw key codes from a global
...
In this chapter, we make those boundaries precise. We begin with the
platform events that arrive at the engine's edge (window focus, display
changes, device connections, raw key and axis data) and show how the
INPUT phase quarantines them into a single consistent snapshot. We then
build from that snapshot upward through device polling, action mapping,
context stacks, and the fixed-tick intent pattern. Along the way we
address mouse capture, controller dead zones, replay, and cross-platform
...
\item
  ...
  built from \texttt{ActionMap}, \texttt{ActionDef}, and
  \texttt{Binding} types, with composite bindings and dead zones.
\item
...
\item
  \textbf{The Input Foundation for Replay:} Why the polled snapshot is
  the input-side prerequisite for deterministic replay, and how Chapter
  22 builds on it.
\item
...
\section{Platform Events Are Engine Input}\label{sec-platform-events}

A game engine sits between the operating system and the game, and the
operating system delivers a continuous, irregular stream of platform
events: window resize notifications, keyboard interrupts, mouse
movements, controller connections, display-scale changes, focus
transitions, and more. These events arrive at the operating system's own
cadence: some are batched, some are immediate, some are synthesized by
the window manager, and their ordering guarantees vary by platform. The
engine's first responsibility is to receive this stream and convert it
into something the game can consume reliably.
...
as an immutable, consistent view. Raw platform events are irregular,
asynchronous, and tied to OS-specific types. Once they cross the INPUT
boundary, they become a deterministic, typed, engine-owned snapshot. No
platform type leaks past this point.

\emph{All platform input is absorbed at a single point in the frame,
converted into engine-owned types, and frozen into an immutable snapshot
before any game system runs}. This is the \textbf{quarantine principle}.
No game code ever reads a raw OS event. No platform type crosses the
boundary. Every system in the tick sees the same input, because they all
read from the same frozen snapshot. Figure \ref{fig:input-quarantine}
illustrates the quarantine principle as the boundary between the OS
...
The quarantine principle is the standard modern answer to a problem that
every engine faces. Unreal Engine's \texttt{FSlateApplication} processes
platform messages in a dedicated input-processing pass and populates
\texttt{FInputState} before any gameplay tick begins. Unity's
\texttt{InputSystem} captures device state at frame start. Godot's
\texttt{Input} singleton processes OS events and exposes polled state
through methods like \texttt{is\_action\_pressed()}. The implementation
details differ, but the principle is universal: absorb the OS's
callback-driven model at the boundary and expose a polled-snapshot model
to the game.

In Smallworld, the quarantine is a single function that runs at the top
of every tick. Listing~\ref{lst-quarantine} shows the drain-and-freeze
step: previous state is rotated, the platform event buffer is drained
into engine types, and single-frame transitions like ``just pressed'' or
``just released are computed by diffing the two frames.

\floattop

\begin{codelisting}

\caption{\label{lst-quarantine}The quarantine: drain platform events and
freeze the snapshot}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{fn}\NormalTok{ quarantine(input}\OperatorTok{:} \OperatorTok{\&}\KeywordTok{mut}\NormalTok{ Input}\OperatorTok{,}\NormalTok{ adapter}\OperatorTok{:} \OperatorTok{\&}\KeywordTok{mut}\NormalTok{ PlatformAdapter) }\OperatorTok{\{}
\NormalTok{  input}\OperatorTok{.}\NormalTok{keyboard}\OperatorTok{.}\NormalTok{previous }\OperatorTok{=}\NormalTok{ input}\OperatorTok{.}\NormalTok{keyboard}\OperatorTok{.}\NormalTok{current}\OperatorTok{;}
\NormalTok{  input}\OperatorTok{.}\NormalTok{mouse}\OperatorTok{.}\NormalTok{prev\_buttons }\OperatorTok{=}\NormalTok{ input}\OperatorTok{.}\NormalTok{mouse}\OperatorTok{.}\NormalTok{buttons}\OperatorTok{;}
\NormalTok{  input}\OperatorTok{.}\NormalTok{mouse}\OperatorTok{.}\NormalTok{delta }\OperatorTok{=} \PreprocessorTok{Vec2::}\NormalTok{ZERO}\OperatorTok{;}
\NormalTok{  input}\OperatorTok{.}\NormalTok{mouse}\OperatorTok{.}\NormalTok{scroll }\OperatorTok{=} \DecValTok{0.0}\OperatorTok{;}

  \ControlFlowTok{for}\NormalTok{ event }\KeywordTok{in}\NormalTok{ adapter}\OperatorTok{.}\NormalTok{drain() }\OperatorTok{\{}
    \ControlFlowTok{match}\NormalTok{ event }\OperatorTok{\{}
      \PreprocessorTok{PlatformEvent::}\NormalTok{Key }\OperatorTok{\{}\NormalTok{ code}\OperatorTok{,}\NormalTok{ pressed }\OperatorTok{\}} \OperatorTok{=\textgreater{}} \OperatorTok{\{}
\NormalTok{        input}\OperatorTok{.}\NormalTok{keyboard}\OperatorTok{.}\NormalTok{current}\OperatorTok{.}\NormalTok{set(code}\OperatorTok{,}\NormalTok{ pressed)}\OperatorTok{;}
      \OperatorTok{\}}
      \PreprocessorTok{PlatformEvent::}\NormalTok{MouseMotion(d) }\OperatorTok{=\textgreater{}} \OperatorTok{\{}
        \CommentTok{// accumulate, never replace}
\NormalTok{        input}\OperatorTok{.}\NormalTok{mouse}\OperatorTok{.}\NormalTok{delta }\OperatorTok{+=}\NormalTok{ d}\OperatorTok{;}
      \OperatorTok{\}}
      \PreprocessorTok{PlatformEvent::}\NormalTok{MouseButton }\OperatorTok{\{}\NormalTok{ btn}\OperatorTok{,}\NormalTok{ pressed }\OperatorTok{\}} \OperatorTok{=\textgreater{}} \OperatorTok{\{}
\NormalTok{        input}\OperatorTok{.}\NormalTok{mouse}\OperatorTok{.}\NormalTok{buttons}\OperatorTok{.}\NormalTok{set(btn}\OperatorTok{,}\NormalTok{ pressed)}\OperatorTok{;}
      \OperatorTok{\}}
      \PreprocessorTok{PlatformEvent::}\NormalTok{Scroll(s) }\OperatorTok{=\textgreater{}} \OperatorTok{\{}
\NormalTok{        input}\OperatorTok{.}\NormalTok{mouse}\OperatorTok{.}\NormalTok{scroll }\OperatorTok{+=}\NormalTok{ s}\OperatorTok{;}
      \OperatorTok{\}}
      \PreprocessorTok{PlatformEvent::}\NormalTok{ControllerAxis }\OperatorTok{\{}\NormalTok{ id}\OperatorTok{,}\NormalTok{ axis}\OperatorTok{,}\NormalTok{ value }\OperatorTok{\}} \OperatorTok{=\textgreater{}} \OperatorTok{\{}
        \ControlFlowTok{if} \KeywordTok{let} \ConstantTok{Some}\NormalTok{(c) }\OperatorTok{=} \OperatorTok{\&}\KeywordTok{mut}\NormalTok{ input}\OperatorTok{.}\NormalTok{controllers[id] }\OperatorTok{\{}
\NormalTok{          c}\OperatorTok{.}\NormalTok{set\_axis(axis}\OperatorTok{,}\NormalTok{ value)}\OperatorTok{;}
        \OperatorTok{\}}
      \OperatorTok{\}}
      \CommentTok{// ... window events update WindowState, not Input}
    \OperatorTok{\}}
  \OperatorTok{\}}
\NormalTok{  input}\OperatorTok{.}\NormalTok{keyboard}\OperatorTok{.}\NormalTok{compute\_edges()}\OperatorTok{;} \CommentTok{// pressed = current \& !previous}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
...
Note what happens inside the loop: mouse deltas are accumulated
(summed), not replaced. Keyboard state is written into a current-frame
bitfield while the previous frame's bitfield is preserved for edge
computation. Controller axes are written directly (they are state, not
deltas). Platform types (\texttt{PlatformEvent}) exist only inside this
function. By the time it returns, the \texttt{Input} struct contains
only engine-owned values.

\subsection{The Platform Adapter}\label{the-platform-adapter}

Every game engine needs a component that bridges the gap between the
operating system's native event model and the engine's internal types.
This component, the platform adapter, receives raw events from the OS
windowing system, translates them into engine-owned representations, and
populates the input snapshot. Its scope is deliberately narrow: it
handles only what is genuinely platform-specific at the input boundary.

The depth of this adapter varies significantly across engines, and the
variation reflects the era and target platforms of each engine's design.
Unreal Engine maintains a deep platform-abstraction layer
(\texttt{FGenericPlatformMisc}, \texttt{FGenericApplication},
platform-specific subclasses for Windows, macOS, Linux, and each
console) because it must support platforms with fundamentally different
input models. Unity's native player loop similarly encapsulates platform
differences behind its \texttt{UnityEngine.Input} facade, with native
plugins and platform-specific backends. Godot's \texttt{DisplayServer}
class wraps window management and input for each platform behind a
common interface.

These deep abstractions are not relics. They remain essential for any
engine that ships on consoles, because console platforms expose
proprietary input APIs under NDA that no open-source library can wrap.
An engine targeting PlayStation, Xbox, or Switch must provide a
platform-specific adapter backend for each, and that backend must
conform to the platform holder's SDK and certification requirements. The
deep abstraction layer exists precisely so that the engine above it does
not need to know which backend is active.

For an engine that targets desktop platforms (Windows, macOS, Linux),
mature cross-platform libraries can absorb much of the platform-specific
work that a deep abstraction would otherwise need to handle. For
instance, Smallworld uses \texttt{winit} as its desktop platform adapter
- a Rust crate that provides cross-platform window creation, event
...
delivers events one at a time through a callback, but the engine needs a
polled snapshot. \texttt{winit} identifies keys by platform-dependent
scan codes and logical key values, but the engine needs a stable
\texttt{KeyCode} enum. \texttt{winit} reports mouse position in window
coordinates, yet the engine may need to track deltas, accumulate scroll,
or enforce cursor capture. And \texttt{winit} knows nothing about
controllers, so the engine pairs it with \texttt{gilrs} or a similar
library for gamepad input. A console port would replace \texttt{winit}
and \texttt{gilrs} entirely with platform-specific backends, but the
\texttt{Input} struct and everything above it would remain unchanged,
because the quarantine principle ensures that no platform type leaks
...
Everything above it speaks the engine's vocabulary. The engine's
stability comes from the types it defines, not from the adapter beneath
them. A console port rewrites the adapter against that platform's SDK,
but the \texttt{Input} struct, the action system, the context stack, and
every game system that reads input remains unchanged.

\subsection{Window and Display State}\label{window-and-display-state}
...
Every engine that separates input from platform state makes this
distinction explicit. Unity exposes window and display information
through \texttt{Screen.width}, \texttt{Screen.height},
\texttt{Screen.dpi}, and \texttt{Application.isFocused}, all separate
from the \texttt{Input} or \texttt{InputSystem} classes. Unreal provides
\texttt{FViewportClient} and \texttt{FGenericWindow} for window state,
while input flows through \texttt{UPlayerInput} and the Enhanced Input
system. Godot splits the concerns between \texttt{DisplayServer} (window
dimensions, screen info) and \texttt{Input} (device state).

The separation is not accidental: window state changes infrequently and
has lifecycle consequences (a resize triggers surface reconfiguration on
the GPU), while input state changes every frame and drives gameplay
logic. Collapsing them into a single structure would obscure this
difference and tempt game code to poll window dimensions at the same
frequency as key state, which is wasteful and conceptually wrong.
...
Smallworld captures platform state in a dedicated \texttt{WindowState}
struct as shown in Listing~\ref{lst-window-state}, which carries the
physical dimensions, scale factor, focus flag, and display mode.
...
\section{From Device Signals to Game Intent}\label{sec-device-to-intent}

With the quarantine principle in place, the engine has a clean snapshot
of device state for this frame. The next question is how game systems
consume it. Smallworld provides two layers. The first is a raw
device-polling API that exposes held/pressed/released semantics for
every key, button, and axis. The second is an action-mapping layer that
translates device signals into named, rebindable, device-agnostic game
actions. The raw layer exists for tools, debug input, and the rare
system that genuinely needs device-level detail. The action layer is the
standard game-facing API.

\subsection{The Raw Polling API}\label{the-raw-polling-api}

Beneath any action-mapping or intent layer, every engine provides a raw
device-polling API: a way to ask, for any key, button, or axis, ``what
is its state right now?'' This layer exists because not every consumer
of input needs the abstraction of named actions. Debug tools, editor
viewports, profiling overlays, and the occasional system that genuinely
needs device-level detail all query raw device state directly. The layer
is also the foundation on which higher abstractions are built: the
action mapping system ultimately resolves its queries by reading raw
state from the snapshot.

The shape of this API is consistent across engines. Unity's
\texttt{Keyboard.current{[}Key.W{]}.isPressed} and
\texttt{Mouse.current.delta.ReadValue()} query device state directly
from the current frame's snapshot. Godot's
\texttt{Input.is\_key\_pressed()} and \texttt{Input.get\_axis()} do the
same through a singleton. Unreal's
\texttt{APlayerController::IsInputKeyDown()} reads raw key state beneath
the Enhanced Input layer. In each case, the API is polled (the game asks
questions rather than registering callbacks) and frame-coherent (every
query within a tick returns values from the same snapshot).

Smallworld's raw polling API follows this consensus.
Listing~\ref{lst-input-struct} defines the \texttt{Input} struct, which
holds per-frame state for the keyboard, mouse, and all connected
controllers.
...
\NormalTok{    controllers}\OperatorTok{:} \DataTypeTok{Vec}\OperatorTok{\textless{}}\NormalTok{ControllerState}\OperatorTok{\textgreater{},}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
...
The struct carries the complete device state for one frame. Game code
queries it through methods like \texttt{key\_held},
\texttt{key\_pressed}, \texttt{mouse\_delta}, and \texttt{controller}.
The full query API (raw device queries, action queries, and fixed-tick
variants) is listed in Listing~\ref{lst-action-queries} after the action
mapping layer is introduced.
...
A key can be in one of two states: up or down. An \textbf{edge} is the
single-frame transition between them: the moment a key goes from up to
down (a \textbf{press edge}) or from down to up (a \textbf{release
edge}). Edges are distinct from state. State tells you \emph{what is
true right now}. An edge tells you \emph{what changed this frame}.

The distinction matters because gameplay actions fall into two
categories: continuous actions that care about state (``is the player
holding the shield?'') and discrete actions that care about transitions
(``did the player just press jump?'').

Confusing the two is one of the most common input bugs in game
development. A jump system that reads ``is the spacebar down?'' instead
of ``was the spacebar pressed this frame?'' will apply the jump impulse
on every frame the key is held, launching the character into orbit. A
weapon system that reads ``pressed'' when it should read ``held'' will
fire exactly once and then stop, even if the player is holding the
trigger.

These bugs are easy to write and surprisingly hard to diagnose, because
the system \emph{works} in the common case (a quick tap) and fails only
when timing matters. The polling API exposes three queries that make the
...
\item
  ...
  \texttt{false} on the frame after release. This is the state query,
  the right choice for continuous actions: moving a character, holding a
  shield, charging a weapon.
\item
  \textbf{\texttt{key\_pressed(key)}} returns \texttt{true} only on the
  single frame when the key transitions from up to down: the press edge.
  It fires once and then is \texttt{false} until the key is released and
  pressed again. This is the right query for discrete actions: jumping,
  firing, opening a menu.
\item
  \textbf{\texttt{key\_released(key)}} returns \texttt{true} only on the
  single frame when the key transitions from down to up: the release
  edge. This is the right query for actions that trigger on release:
...
that supports edge queries. Unity's legacy \texttt{Input.GetButtonDown}
and \texttt{Input.GetButtonUp} perform exactly this comparison, as do
Godot's \texttt{is\_action\_just\_pressed()} and
\texttt{is\_action\_just\_released()}. Unreal's Enhanced Input system
tracks transitions through its \texttt{ETriggerEvent} enum,
distinguishing \texttt{Started}, \texttt{Ongoing}, and
\texttt{Completed} phases.

The cost is one additional bit per key per frame: negligible. The
benefit is that game systems never need to track previous state
themselves, which eliminates a class of bugs where two systems
independently track the same key and disagree about its edge timing.

Edge detection is also the bridge to the two more complex problems
discussed later in this chapter: action mapping (which must propagate
edges through the binding table so that \texttt{action\_pressed("jump")}
fires exactly once, not once per bound key) and the fixed-tick edge
problem (where edges interact with the accumulator in ways that require
the intent pattern to resolve correctly).
...
pressed/released edges. Analog input is where the gap between ``works in
a tutorial'' and ``works in a shipped game'' is widest, because the
physical characteristics of analog devices introduce noise, drift, and
precision issues that digital keys simply do not have.

\textbf{Mouse delta and scroll} are accumulated values: the INPUT phase
sums all motion or scroll events received since the previous snapshot
and reports the total as a single \texttt{Vec2} (delta) or \texttt{f32}
(scroll, positive for up, negative for down). \textbf{Mouse position} is
a state query: it reflects the last reported cursor location in window
coordinates, not a sum. The accumulation model, its interaction with
high-polling-rate mice, and cursor capture are discussed in detail in
the Mouse Delta Accumulation and Cursor Capture section below.

\textbf{Controller sticks and triggers} present a different set of
challenges. Sticks report a \texttt{Vec2} in the range
\texttt{{[}-1.0,\ 1.0{]}} per axis. Triggers report an \texttt{f32} in
the range \texttt{{[}0.0,\ 1.0{]}}. These values are sampled directly
from the controller's current state.

But physical sticks are imperfect mechanisms: they drift, they have
manufacturing tolerances, and they wear over time. The Nintendo Switch
Joy-Con drift issue, which became so widespread that it prompted a
class-action lawsuit and a free repair program, is the most visible
example of a problem that every controller exhibits to some degree. A
brand-new Xbox controller may report \texttt{(0.02,\ -0.01)} when the
stick is centered. An older DualSense may drift further. Without
compensation, the game would register constant low-magnitude movement
even when the player is not touching the stick. Dead zones (discussed in
the action mapping section below) address this at the binding level, but
the raw polling API reports the unfiltered value, leaving the choice of
threshold to the consumer.

Listing~\ref{lst-controller-state} defines the per-controller types.
\texttt{ControllerId} captures the USB HID descriptor fields that
uniquely identify a physical device: vendor and product IDs (assigned by
the USB Implementers Forum) and an optional serial number (not all
devices report one). \texttt{DeviceFilter} uses this identity in two
modes: \texttt{Any} matches any connected controller (the right default
for a gamepad game), while \texttt{Specific} pins a binding to one
physical device (the right choice for a simulation cockpit where each
USB device has a distinct role). \texttt{ControllerState} itself carries
the identity alongside the analog and digital state: two sticks as
\texttt{Vec2} values in \texttt{{[}-1.0,\ 1.0{]}}, two triggers as
\texttt{f32} values in \texttt{{[}0.0,\ 1.0{]}}, and a bitfield of
buttons.

\begin{codelisting}

\caption{\label{lst-controller-state}ControllerState: identity and
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{struct}\NormalTok{ ControllerId }\OperatorTok{\{}
\NormalTok{    vendor}\OperatorTok{:}  \DataTypeTok{u16}\OperatorTok{,}
\NormalTok{    product}\OperatorTok{:} \DataTypeTok{u16}\OperatorTok{,}
\NormalTok{    serial}\OperatorTok{:}  \DataTypeTok{Option}\OperatorTok{\textless{}}\DataTypeTok{String}\OperatorTok{\textgreater{},}
\OperatorTok{\}}

\KeywordTok{enum}\NormalTok{ DeviceFilter }\OperatorTok{\{}
    \BuiltInTok{Any}\OperatorTok{,}                             \CommentTok{// match any connected controller}
\NormalTok{    Specific(ControllerId)}\OperatorTok{,}          \CommentTok{// match a specific physical device}
\OperatorTok{\}}

\KeywordTok{struct}\NormalTok{ ControllerState }\OperatorTok{\{}
\NormalTok{    id}\OperatorTok{:}\NormalTok{            ControllerId}\OperatorTok{,}
...
on consoles expects four gamepads sharing a screen, a convention that
has been stable since the arcade era. Simulation enthusiasts on PC may
connect a stick, a throttle quadrant, rudder pedals, and a button box
simultaneously, each recognized by the OS as a separate HID controller.
These are different use cases, but they place the same demand on the
input system: it must track the state of multiple controllers
concurrently and let game code address them by stable index.

The \texttt{Input} struct stores connected controllers in a
\texttt{Vec\textless{}ControllerState\textgreater{}}. The quarantine
phase populates this vector from the OS device list each frame. Slot
indices are stable within a session (a controller that appears at index
2 stays at index 2 as long as it remains connected), but they are
\emph{not} stable across sessions. If a player connects their throttle
before their stick on Tuesday but reverses the order on Wednesday, the
OS may assign different indices. This is a well-known problem: XInput on
Windows assigns gamepad indices 0-3 in connection order, and a
disconnect/reconnect can shuffle them mid-session.

The solution is device identity. Each USB HID controller reports a
vendor ID, product ID, and (on some devices) a serial number. The
engine's controller library (\texttt{gilrs} on desktop,
platform-specific APIs on consoles) exposes these identifiers, and the
action mapping layer uses them to bind actions to specific physical
devices rather than to slot indices. A flight sim binding that says
``throttle axis on the Thrustmaster TWCS (vendor 0x044F, product
0xB687)'' survives reconnection in any order. Device assignments persist
to \texttt{user://} alongside key rebindings, so the player configures
their setup once.

A dynamic collection is the right choice for the raw controller list
because the number of connected controllers varies widely across use
cases. A console game with four gamepads and a PC cockpit setup with six
USB HID devices both work without recompiling the engine or raising an
arbitrary constant. The \texttt{Vec} is allocated once per frame during
the quarantine (not in a hot loop), and controller counts are small
enough that the cost is negligible. The \texttt{Vec} is a plain owned
value: it is \texttt{Send}, \texttt{\textquotesingle{}static}, and
serializable without lifetime concerns.
...
afterthought: Quake (1996) let players remap keys through console
commands like \texttt{bind\ W\ +forward}, but the binding system was
bolted onto a fixed set of text commands, not a general abstraction for
game intent.
...
The evolution accelerated as rebinding shifted from a power-user
convenience to a certification and legal requirement. The 21st Century
Communications and Video Accessibility Act (CVAA) in the United States,
which began enforcement for games in 2019, requires that games with
communication features provide accessible input options, including
remapping.

Microsoft's Xbox Accessibility Guidelines (XAGs), which influence
certification for Xbox and Game Pass titles, explicitly recommend full
input remapping. Sony's PlayStation certification includes similar
provisions. Studios like Naughty Dog demonstrated with \emph{The Last of
Us Part II} (2020) that comprehensive accessibility, including full
remapping, one-handed play modes, and custom control schemes, could ship
in a AAA title without compromising the core experience.

Rebinding is no longer a convenience but a certification prerequisite on
major platforms and a legal requirement in some jurisdictions. The
architectural consequence is that rebinding must be a data operation on
the action map, not a code change.
...
\NormalTok{    kind}\OperatorTok{:}\NormalTok{ ActionKind}\OperatorTok{,}                \CommentTok{// Button | Axis1d | Axis2d}
...
\NormalTok{    Axis1d}\OperatorTok{,}
\NormalTok{    Axis2d}\OperatorTok{,}
\NormalTok{    Axis3d}\OperatorTok{,}
\OperatorTok{\}}
...
\NormalTok{    MouseMotion}\OperatorTok{,}                                       \CommentTok{// Axis2d}
\NormalTok{    ControllerButton }\OperatorTok{\{}\NormalTok{ device}\OperatorTok{:}\NormalTok{ DeviceFilter}\OperatorTok{,}\NormalTok{ button}\OperatorTok{:} \DataTypeTok{u8} \OperatorTok{\},}
\NormalTok{    ControllerAxis  }\OperatorTok{\{}\NormalTok{ device}\OperatorTok{:}\NormalTok{ DeviceFilter}\OperatorTok{,}\NormalTok{ axis}\OperatorTok{:} \DataTypeTok{u8}\OperatorTok{,}\NormalTok{ dead\_zone}\OperatorTok{:} \DataTypeTok{f32} \OperatorTok{\},}
\NormalTok{    Composite }\OperatorTok{\{}\NormalTok{ axes}\OperatorTok{:} \DataTypeTok{Vec}\OperatorTok{\textless{}}\NormalTok{(KeyCode}\OperatorTok{,}\NormalTok{ KeyCode)}\OperatorTok{\textgreater{}} \OperatorTok{\},} \CommentTok{// (positive, negative) per axis}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
...
Listing~\ref{lst-action-types} shows how Smallworld defines the three
types that compose the mapping: \texttt{ActionMap}, \texttt{ActionDef}
with its \texttt{ActionKind}, and the \texttt{Binding} enum.
...
type: \texttt{Button} for digital on/off, \texttt{Axis1d} for a single
continuous scalar (trigger pull, throttle), \texttt{Axis2d} for a
two-dimensional continuous value (stick direction, mouse look), and
\texttt{Axis3d} for a three-dimensional continuous value (a
\texttt{Vec3}).

\texttt{Axis3d} exists because a modern engine cannot treat 3D spatial
input as an afterthought. VR controllers report position as a continuous
3D vector. 6DOF desktop controllers like the 3Dconnexion SpaceMouse
report three translation axes and three rotation axes, each naturally
expressed as an \texttt{Axis3d} action. Decomposing these into three
independent \texttt{Axis1d} actions is technically possible, but it
forces the game to reassemble a spatial value that the hardware already
provides as a unit, and it breaks the action system's promise that a
single query returns a complete, typed answer. With \texttt{Axis3d}, a
VR game reads \texttt{axis3d("hand\_position")} and gets a \texttt{Vec3}
the same way a gamepad game reads \texttt{axis2d("move")} and gets a
\texttt{Vec2}.

The four value types cover the full range of input dimensionality
without being open-ended. A flight simulator with a HOTAS (hands-on
throttle and stick) setup does not need a richer \texttt{ActionKind}, it
needs more named actions. Pitch, roll, yaw, throttle, mixture, propeller
RPM, and trim are each a separate \texttt{Axis1d} action, each bound to
a different physical axis on whichever controller the player has
assigned. The \texttt{ControllerAxis} binding identifies the source by
device filter and axis index. When \texttt{device} is
\texttt{DeviceFilter::Any}, the binding matches any connected
controller, which is the right default for a gamepad game where any
controller should work. When \texttt{device} is
\texttt{DeviceFilter::Specific(id)}, the binding is pinned to a specific
physical device by its HID identity, which is the right choice for a
simulation cockpit where each USB device has a distinct role. The system
scales by adding actions and bindings, not by adding new kinds.

Each action can have multiple bindings simultaneously. Consider a
``move'' action of kind \texttt{Axis2d}. In a legacy engine like Quake,
movement was four separate boolean actions: \texttt{+forward},
\texttt{+back}, \texttt{+moveleft}, \texttt{+moveright}. The game
checked each independently and summed the results into a direction. This
worked for keyboards, but it could not represent the partial deflection
of an analog stick, and it forced every character controller to
reconstruct a direction vector from four booleans. Modern engines
replaced this pattern with a single multi-dimensional action. UE5's
Enhanced Input defines a ``Move'' \texttt{InputAction} with a
\texttt{FVector2D} value type. Unity's Input System defines a ``Move''
action with a \texttt{Vector2} control type. The game reads one value
and gets a continuous direction vector, regardless of whether it came
from a stick, a touchscreen virtual joystick, or four digital keys
reassembled by a composite binding.

Smallworld follows the same pattern. A ``move'' action of kind
\texttt{Axis2d} might bind to
\texttt{Composite\ \{\ axes:\ {[}(W,\ S),\ (A,\ D){]}\ \}} for keyboard
and
\texttt{ControllerAxis\ \{\ device:\ Any,\ axis:\ 0,\ dead\_zone:\ 0.15\ \}}
for any connected gamepad. The engine evaluates all bindings and
produces the appropriate result: for a \texttt{Button} action, any
active binding triggers the action, and for an \texttt{Axis2d} action,
the engine returns the most recent non-zero contributor (with analog
sources taking priority over digital composites when both are active).
The game reads \texttt{axis2d("move")} and gets a \texttt{Vec2} with
magnitude in \texttt{{[}0.0,\ 1.0{]}}, pointing in the direction the
player intends to move. It never asks which key is down or which stick
is deflected.

Listing~\ref{lst-action-queries} lists the complete query API on
\texttt{Input}. The top group provides raw device queries (the same
methods introduced in Section~\ref{sec-platform-events}). The middle
group provides action queries that resolve through the binding table:
\texttt{action\_held}, \texttt{action\_pressed}, and
\texttt{action\_released} mirror their raw counterparts but operate on
named actions. The \texttt{fixed\_action\_pressed} and
\texttt{fixed\_action\_released} variants are consumed-once latches
designed for the FIXED phase, explained in the Intent Pattern section
below. The axis readers return continuous values at the appropriate
dimensionality. The stack operations manage the context stack.

\begin{codelisting}

\caption{\label{lst-action-queries}The complete Input query API}
...
\KeywordTok{impl}\NormalTok{ Input }\OperatorTok{\{}
    ...
    \KeywordTok{fn}\NormalTok{ key\_pressed(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ key}\OperatorTok{:}\NormalTok{ KeyCode) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{bool}\OperatorTok{;}
    \KeywordTok{fn}\NormalTok{ key\_released(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ key}\OperatorTok{:}\NormalTok{ KeyCode) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{bool}\OperatorTok{;}
    ...
    \CommentTok{// Action queries (resolved through bindings)}
    ...
    \KeywordTok{fn}\NormalTok{ fixed\_action\_pressed(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ name}\OperatorTok{:} \OperatorTok{\&}\DataTypeTok{str}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{bool}\OperatorTok{;}
    \KeywordTok{fn}\NormalTok{ fixed\_action\_released(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ name}\OperatorTok{:} \OperatorTok{\&}\DataTypeTok{str}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{bool}\OperatorTok{;}
    \KeywordTok{fn}\NormalTok{ axis1d(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ name}\OperatorTok{:} \OperatorTok{\&}\DataTypeTok{str}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{f32}\OperatorTok{;}
    \KeywordTok{fn}\NormalTok{ axis2d(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ name}\OperatorTok{:} \OperatorTok{\&}\DataTypeTok{str}\NormalTok{) }\OperatorTok{{-}\textgreater{}}\NormalTok{ Vec2}\OperatorTok{;}
    \KeywordTok{fn}\NormalTok{ axis3d(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ name}\OperatorTok{:} \OperatorTok{\&}\DataTypeTok{str}\NormalTok{) }\OperatorTok{{-}\textgreater{}}\NormalTok{ Vec3}\OperatorTok{;}

    \CommentTok{// Context stack}
...
that was written against \texttt{key\_held(KeyCode::W)} to move forward
can be migrated to \texttt{axis2d("move")}, and it gains rebindability,
device agnosticism, and context-stack awareness for free.

Two binding types deserve special attention because they bridge the gap
between digital and analog input:

\textbf{Composite} converts digital keys into a continuous axis value of
any dimensionality. Each entry in the \texttt{axes} list is a
\texttt{(positive,\ negative)} key pair that contributes one axis. Two
pairs produce an \texttt{Axis2d} (WASD:
\texttt{{[}(W,\ S),\ (A,\ D){]}}). Three pairs produce an
\texttt{Axis3d} (WASD + Space/Ctrl for a free-camera flythrough:
\texttt{{[}(W,\ S),\ (A,\ D),\ (Space,\ LCtrl){]}}). The engine
validates at bind time that the number of pairs matches the action's
\texttt{ActionKind} dimensionality.

When multiple keys are active simultaneously, the resulting vector is
normalized to unit length. Without normalization, diagonal movement
would be approximately 41\% faster than cardinal movement in 2D, and the
problem compounds in 3D where a player pressing three axes at once would
move \(\sqrt{3} \approx 1.73\) times faster than along a single axis.
The normalization happens inside the binding evaluation, so game code
that reads \texttt{axis2d("move")} or \texttt{axis3d("fly")} always
receives a vector with magnitude in \texttt{{[}0.0,\ 1.0{]}}.
...
concern. The game receives a clean \texttt{{[}0.0,\ 1.0{]}} value and
...
``jump'' and ``fire,'' the menu uses ``navigate'' and ``select,'' and
the console uses text entry. These vocabularies must coexist, and at any
given moment exactly one (or a controlled subset) should be active.

The naive solution is a set of boolean flags like
\texttt{is\_menu\_open}, \texttt{is\_console\_active}, or
\texttt{is\_text\_field\_focused}. Every system checks the relevant
flags before reading input. This approach scales poorly: each new
activity adds a flag that every existing system must check, and it
encodes priority implicitly. If the console and the menu are both open,
which one receives input? The answer depends on the order of the flag
checks, which is scattered across unrelated systems. Anyone who has
played a game where opening the inventory did not prevent the character
from moving, or where pressing Escape closed a dialog \emph{and} opened
the pause menu simultaneously, has experienced the consequences of
ad-hoc input routing.

Every major engine has arrived at some form of structured input routing
to solve this problem, though the mechanisms differ. Unreal Engine 5's
Enhanced Input system uses \texttt{InputMappingContext} objects with
explicit integer priorities: higher-priority contexts consume input
first, and a ``consumes input'' flag determines whether lower-priority
contexts see the event at all. Unity's Input System allows action maps
to be enabled and disabled independently, and the game manages which
maps are active at any given moment. Godot propagates input events
through the scene tree, with each node given the opportunity to
``handle'' an event and stop propagation via
\texttt{\_unhandled\_input()}.

Each approach encodes the same fundamental idea: input routing is a
structured priority problem, not a collection of per-system boolean
checks.

Smallworld's version of this structure is a \textbf{context stack}.
Active action maps are pushed onto a stack with the topmost map
receiving input first. Each map carries a \texttt{passthrough} flag that
controls whether maps below it on the stack also receive input:
...
Figure \ref{fig:action-stack} illustrates a typical context stack during
a pause menu. At the bottom sits the \texttt{gameplay} map (with actions
like ``move'', ``jump'', and ``fire''). Above it, the \texttt{ui} map
has been pushed with \texttt{passthrough:\ false}, blocking all gameplay
actions. The player's key presses resolve against the \texttt{ui} map
only: ``navigate'' and ``select'' are active, but ``jump'' and ``fire''
are invisible. When the player closes the menu, the \texttt{ui} map is
popped, and the \texttt{gameplay} map becomes the top of the stack
again, restoring full gameplay input without any explicit re-enabling
step.
...
\item
  ...
  responsible for pushing its map and the code that deactivates it is
...
The connection to the UI framework deserves emphasis. In Chapter 23 we
describe UI architecture in detail, but the core input contract is
established here: UI consumes input ahead of game systems. This is not a
special case or a priority number. It is a structural consequence of the
stack: the UI map sits above the gameplay map, and its
\texttt{passthrough:\ false} flag blocks gameplay actions when the UI is
capturing input.

\subsection{Rebinding and Persistence}\label{rebinding-and-persistence}

Because action maps are \textbf{assets} (loaded through the asset
pipeline, identified by name, modifiable at runtime), rebinding is a
data operation rather than a code change. The settings screen reads the
current action map, displays the bindings, and lets the player modify
them. No special rebinding API exists: the same \texttt{ActionMap} and
\texttt{Binding} types that define actions at development time are the
types the player edits at runtime.
...
Enter, the overlay replaces that single binding and all other bindings
remain at their defaults. Device assignments (which physical controller
is bound to which role, via \texttt{DeviceFilter::Specific}) are
persisted in the same overlay, so a simulation player's cockpit setup
survives across sessions.

The overlay model has two useful properties. First, version upgrades are
conflict-free: when a game update adds a new action, the default map
contains the new binding, and the user override (which does not mention
the new action) does not conflict. Both coexist without a migration
step. Second, ``reset to defaults'' is a file deletion: remove the user
override, reload the default map, and rebuild the input state. The
default map is never modified: it is always available as the canonical
fallback.
...
\texttt{Binding} entries, and writes the result to \texttt{user://}. The
engine provides the data types and the persistence path. The game
provides the UI. This separation ensures that the rebinding interface
matches the game's visual style rather than being a generic engine
dialog.
...
\subsection{The Polled Snapshot and Frame
Timing}\label{the-polled-snapshot-and-frame-timing}

Section~\ref{sec-platform-events} established that the quarantine
principle converts pushed OS events into a polled snapshot. The
distinction is worth revisiting briefly in the context of frame timing,
because the interaction between the snapshot and the phase lifecycle has
consequences that the quarantine discussion alone does not cover.

The polled model's core advantage is that every query within a tick
returns a value from the same point in time. A system that reads
\texttt{action\_held("fire")} does not need to iterate over an event
queue, handle out-of-order events, or deduplicate events that the OS
delivered twice (a real problem on some platforms with auto-repeat).

A snapshot is also a compact, self-contained record: recording input for
replay means saving one snapshot per frame, while a pushed-event model
would require recording events with sub-frame timestamps and replaying
them in the correct order relative to the frame boundary. The pushed
model is not wrong - it is appropriate for text entry, where character
ordering matters and a snapshot that reports only ``key A is held''
loses information. Smallworld handles text entry as a separate path
(discussed in Section~\ref{sec-cross-platform}), not through the action
system.
...
Between frames, the OS delivers events to \texttt{winit}'s event buffer
at any time: key presses, mouse movements, controller state changes, and
window focus events all accumulate while the engine is busy with the
previous tick's UPDATE, LATE, EXTRACT, and CLEAR phases.

At the start of the next tick, the INPUT phase drains that buffer and
freezes the snapshot. From that point, the snapshot is immutable. The
FIXED phase may read it (with caveats about edges, discussed below). The
UPDATE phase is the primary consumer for input-driven gameplay logic.
The LATE phase benefits from the consistency guarantee but does not
typically read input directly.
...
Edge queries like \texttt{action\_pressed("jump")} return \texttt{true}
for exactly one frame. But the FIXED phase, driven by the accumulator
described in Chapter 4, may run zero, one, or several times during that
frame. If it runs zero times, the edge is never seen and the jump is
lost. If it runs twice, the edge is seen twice and the jump impulse is
applied twice. The player presses jump once, and the character either
does not jump or jumps too high, depending on the frame rate.

This is the \textbf{fixed-tick edge problem}: edges are properties of
the frame, but fixed ticks are properties of the accumulator, and the
two clocks are not synchronized.

The fixed-tick edge problem is the input-side consequence of the
mismatch between accumulator and interpolation described in Chapter 4.
Unity's documentation explicitly warns against reading
\texttt{Input.GetButtonDown} inside \texttt{FixedUpdate}, and the topic
is one of the most persistent questions on Unity's forums. Godot's
documentation calls it out when explaining the relationship between
\texttt{\_input} callbacks and \texttt{\_physics\_process}.

To see why, consider a frame where the player presses the jump button.
...
The left column shows the happy path (one tick per frame). The center
column shows an edge lost to a fast frame. The right column shows an
edge double-fired during a slow frame.

\input{figures/fixed-tick-edge-problem.tex}

\subsection{The Intent Pattern}\label{the-intent-pattern}

The standard solution to the fixed-tick edge problem is the
\textbf{intent pattern}: latch the edge when it fires, and consume the
latch at most once in the fixed-rate tick. The latch persists across
frames, so it survives the zero-tick case (the next fixed tick will see
it). It is cleared on first consumption, so it prevents the double-fire
case.

The pattern has no canonical name in the literature, but it appears
independently in every engine that has confronted the problem. Unity's
community documentation recommends it for \texttt{FixedUpdate} input.
Godot's manual suggests forwarding \texttt{\_input} events into state
variables that \texttt{\_physics\_process} reads.

Most engines leave the intent pattern to game code: the developer
declares a boolean, sets it in the variable-rate update, and clears it
in the fixed-rate tick. This works, but it requires the developer to
know the pattern exists, to implement it correctly for every discrete
action, and to remember to clear the flag after consumption. If the
developer simply calls \texttt{action\_pressed("jump")} inside
\texttt{fixed\_update} without knowing about the mismatch, the bug ships
silently and only surfaces at unusual frame rates.

Smallworld eliminates this class of bug by providing the intent pattern
as a built-in query. The \texttt{Input} struct offers
\texttt{fixed\_action\_pressed} and \texttt{fixed\_action\_released}
alongside the standard edge queries. The engine can do this because it
already knows every action that exists: the game's \texttt{ActionMap}
assets are loaded at startup and contain every \texttt{ActionDef} by
name. During the INPUT phase, after computing raw edges from the device
state, the engine evaluates all button-type action bindings against the
new edges. For any action whose bindings produced an edge that frame,
the engine sets a one-shot latch in the action mapping system, keyed by
action name. \texttt{fixed\_action\_pressed("jump")} looks up that latch
and clears it on first consumption. No game-side \texttt{update} hook is
needed because the latch lives inside the engine's action system, not on
a game struct.

Listing~\ref{lst-intent-pattern} shows the difference. The
engine-managed version is a single query in \texttt{fixed\_update} with
no manual bookkeeping. The manual version, shown for comparison,
requires the game to declare a flag, set it in \texttt{update}, and
clear it in \texttt{fixed\_update}.

\begin{codelisting}

\caption{\label{lst-intent-pattern}The intent pattern: engine-managed
vs.~manual}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// Engine{-}managed: one call, correct by construction}
\KeywordTok{fn}\NormalTok{ fixed\_update(}\OperatorTok{\&}\KeywordTok{mut} \KeywordTok{self}\OperatorTok{,}\NormalTok{ ctx}\OperatorTok{:} \OperatorTok{\&}\KeywordTok{mut}\NormalTok{ GameContext) }\OperatorTok{\{}
    \ControlFlowTok{if}\NormalTok{ ctx}\OperatorTok{.}\NormalTok{input}\OperatorTok{.}\NormalTok{fixed\_action\_pressed(}\StringTok{"jump"}\NormalTok{) }\OperatorTok{\{}
\NormalTok{        apply\_jump\_impulse(ctx}\OperatorTok{,}\NormalTok{ player)}\OperatorTok{;}
    \OperatorTok{\}}
\OperatorTok{\}}

\CommentTok{// Manual equivalent (still available for custom semantics):}
\KeywordTok{fn}\NormalTok{ update(}\OperatorTok{\&}\KeywordTok{mut} \KeywordTok{self}\OperatorTok{,}\NormalTok{ ctx}\OperatorTok{:} \OperatorTok{\&}\NormalTok{GameContext) }\OperatorTok{\{}
...
The manual approach remains available for cases where the game needs
custom consumption semantics: a charged attack that latches on press and
triggers on release across multiple fixed ticks, or a combo system that
accumulates edges over a timing window. But for the common case of
``this discrete action should fire exactly once in the physics tick,''
the engine-managed query is the right default.
...
\vspace{-3mm}\textbf{Why Not Automatically Buffer All Edges for Fixed Ticks?}\vspace{3mm}

The engine provides \texttt{fixed\_action\_pressed} as an opt-in query,
not as a replacement for \texttt{action\_pressed}. Automatically
buffering all edges and replaying them into each fixed tick is a
different and more fragile approach. It introduces a hidden queue
between the input system and the game, it must decide how to handle
multiple edges in a single frame (did the player press and release
between ticks, or press twice?), and it obscures the fundamental
mismatch from the developer. The opt-in query makes the correct pattern
trivial to use without hiding the underlying model.
...
absolute. Unlike a key (which is either up or down) or a stick (which
reports an absolute position), the mouse reports \emph{how far it moved}
since the OS last sampled it. The INPUT phase accumulates all motion
events received since the previous snapshot and reports the sum as a
single \texttt{Vec2}. If three motion events arrive during a frame with
deltas \texttt{(5,\ 0)}, \texttt{(0,\ 3)}, and \texttt{(2,\ -1)}, the
snapshot reports a single \texttt{mouse\_delta()} of \texttt{(7,\ 2)}.
Individual events are not exposed because they would reintroduce
event-ordering concerns inside the snapshot model.

This accumulation model is particularly important for high-polling-rate
mice. Competitive FPS players routinely use mice that report at 1000 Hz
or higher, delivering dozens of motion events during a single 16 ms
frame. If the engine reads only the latest event, it discards the
majority of the player's actual movement. If it exposes individual
events, it forces game systems to iterate over a variable-length queue,
breaking the polled-snapshot model. Accumulation is the only approach
that preserves both the full range of player motion and the simplicity
of a single per-frame value. The difference between a mouse that reports
at 125 Hz and one that reports at 1000 Hz is perceptible in fast
flick-shots, and any engine that drops motion events between polls will
feel less responsive than one that accumulates them.

\textbf{Cursor capture} is the platform-level mechanism that makes
mouse-look work. When the cursor is captured (locked to the window
center, invisible, with raw motion reporting), the game receives pure
deltas without the cursor hitting screen edges. The game requests
capture through the platform adapter, and the adapter configures
\texttt{winit} to lock the cursor and enable raw motion. The captured
state persists until the game releases it, typically when a menu opens,
the window loses focus, or the game pauses. Focus loss automatically
releases capture (the OS may enforce this regardless), and the INPUT
phase detects the change and reports zero deltas until capture is
re-established.

Cursor capture interacts with the context stack in a natural way. When
gameplay is active and the player is controlling a camera, the cursor is
captured and mouse deltas drive look rotation. When the player opens a
menu (pushing a UI context onto the stack), the game releases the cursor
so the player can interact with UI elements. When the menu is dismissed
(popping the UI context), the cursor is recaptured. The context stack
already manages which action map is active and cursor capture state
follows the same transitions, ensuring that the mouse behaves correctly
for each activity without ad-hoc management.

\subsection{The Input Foundation for
Replay}\label{the-input-foundation-for-replay}

The polled-snapshot model has a valuable structural property: each
frame's \texttt{Input} snapshot is a self-contained, serializable value.
Recording input for replay means saving one snapshot per frame, while
replaying means feeding recorded snapshots into the INPUT phase in place
of the live quarantine drain. Combined with Chapter 4's deterministic
fixed timestep, the same sequence of input snapshots produces the same
sequence of world states.

We will revisit replay, recording and determinism as a dedicated topic
in Chapter 24, building on the input foundation established here.

\section{Cross-Platform Policy}\label{sec-cross-platform}
...
discusses three cross-platform concerns that the mechanism enables:
seamless multi-device support, platform-specific conventions, and text
entry.

\subsection{Multi-Device Support and Prompt
Adaptation}\label{multi-device-support-and-prompt-adaptation}

Because the action system evaluates all active bindings from all
connected devices, a player who unplugs a controller and picks up a
keyboard does not need to restart the game or change a setting. If the
player presses W on the keyboard while the controller is idle, the
action resolves from the keyboard binding. If the player then touches
the right stick, the action resolves from the controller binding. The
transition is seamless because game code never asked which device the
input came from.

This seamless switching enables a related feature: on-screen prompt
adaptation. The input system tracks which device last produced a
non-zero input and exposes this as a query, allowing the UI to display
the appropriate prompt icon (``Press A to jump'' on gamepad, ``Press
Space to jump'' on keyboard). This is a presentation concern, not a
logic concern. The action value is the same regardless of the prompt.

Different platforms also have different button conventions for the same
semantic action. These conventions are handled by the binding table: the
platform's default action map uses the conventional button for
``confirm,'' and the player can rebind if desired. Game code never reads
a specific button, because it queries
\texttt{action\_pressed("confirm")} instead.
...
\vspace{-3mm}\textbf{The Confirm Button Problem}\vspace{3mm}

The history of the confirm button illustrates why action mapping exists.
On PlayStation, the confirm button was traditionally Cross in Western
territories and Circle in Japan, a split that persisted for over 25
years. Japanese players who imported Western games (or vice versa) found
that confirm and cancel were swapped, because the game had hard-coded
the button rather than mapping a semantic action. Sony finally
standardized on Cross globally with the PlayStation 5 in 2020, but the
transition was controversial and some Japanese players still prefer
Circle. Meanwhile, Nintendo's Switch places its ``A'' button where
Xbox's ``B'' sits, and vice versa. A game that reads
\texttt{controller\_button(0)} will get a different physical button on
every platform. A game that reads \texttt{action\_pressed("confirm")}
and ships with per-platform default bindings handles all of these
conventions correctly, and a player who disagrees with the platform
default can rebind.
...
engine must support character-by-character input with correct Unicode
handling (including multi-byte characters, emoji, and right-to-left
scripts), Input Method Editors (IME) for languages like Chinese,
...
and composition state (the intermediate string that the IME is building
...
sequence. The IME interaction is inherently event-driven: the OS
...
produces a system that handles neither well. The clean separation keeps
each path simple and correct.

\section{Accessibility}\label{sec-accessibility}

The action mapping and rebinding infrastructure described in
Section~\ref{sec-device-to-intent} provides the motor-input foundation
for accessibility: full rebinding, device-agnostic actions, persistent
user overrides, and \texttt{DeviceFilter} support for adaptive
controllers. But accessibility spans far more than input remapping.
Motor accommodations (hold-to-toggle, sensitivity curves, one-handed
presets), visual accommodations (colorblind modes, font scaling,
high-contrast UI), auditory accommodations (subtitles, visual indicators
for sound cues, mono audio), and cognitive accommodations (difficulty
scaling, navigation assistance, reduced UI complexity) each touch
different engine subsystems and carry their own certification
requirements.

Chapter 25 covers accessibility as a dedicated, cross-cutting topic,
building on the input primitives established here and the UI, audio, and
rendering infrastructure introduced in later chapters.

\section{Input in the GameContext}\label{input-in-the-gamecontext}
...
Listing~\ref{lst-game-context} shows a deliberate asymmetry in
mutability. \texttt{world} and \texttt{assets} are mutable borrows
because game systems legitimately mutate game state and request asset
operations. \texttt{input}, \texttt{time}, and \texttt{window} are
immutable because they describe the external environment, which the game
observes but does not control. The snapshot was frozen during INPUT, and
no game system should be able to alter what other systems see.

The \texttt{push\_map} and \texttt{pop\_map} methods are the exception:
they modify the context stack. Because reading input and modifying the
stack are logically distinct operations (one observes, the other
mutates), the engine routes stack operations through a dedicated method
on \texttt{GameContext} rather than through the immutable \texttt{input}
borrow, ensuring that reading and writing never conflict.
...
\item
  ...
  (\texttt{action\_pressed("jump")}, \texttt{axis2d("move")}) adds
  named, rebindable, device-agnostic actions defined as data-driven
  \texttt{ActionMap} assets. The raw layer exists for tools and debug,
  while the action layer is the standard game-facing API.
\item
  ...
  a blocking UI map, and gameplay actions beneath it are silenced. This
...
\item
  ...
  more than once. The engine provides \texttt{fixed\_action\_pressed}
  and \texttt{fixed\_action\_released} as consumed-once latches that
  handle this correctly by construction. The manual intent-flag pattern
  remains available for custom consumption semantics.
\item
  \textbf{The input foundation for replay.} The polled snapshot is a
  compact, serializable, per-frame record. Combined with Chapter 4's
  deterministic fixed timestep, it provides the input-side foundation
  for replay, automated testing, and network input prediction. Chapter
  24 covers replay, recording, and determinism as a dedicated topic.
\item
  \textbf{Cross-platform policy.} One action vocabulary serves all
  devices. Seamless device switching and prompt adaptation fall out of
  the binding system. Platform button conventions are handled by
  per-platform default action maps. IME and text composition travel a
  separate event path to the focused UI widget, because text input and
  action input are different in kind.
\item
  \textbf{Accessibility.} The input system provides the
  motor-accessibility foundation (rebinding, device-agnostic actions,
  adaptive controller support). Chapter 25 covers accessibility as a
  cross-cutting concern spanning input, UI, audio, and visual design.
\end{enumerate}
...
\item
  A \texttt{Composite} binding maps key pairs to an axis value. Why must
  the resulting vector be normalized? Calculate the magnitude of the
  unnormalized vector when two of two axes are active simultaneously (2D
  case) and when three of three axes are active simultaneously (3D
  case).
\item
...
\item
  The \texttt{Input} struct uses a
  \texttt{Vec\textless{}ControllerState\textgreater{}} for controllers
  and identifies each by \texttt{ControllerId} (vendor, product,
  serial). Why is device identity important for the binding system? What
  would go wrong if bindings referenced controllers by slot index alone?
\item
...
Chapter 5 established the engine's input system: a polled snapshot that
converts irregular platform events into a stable, frame-synchronized
vocabulary of player intent. That system relies on several mechanisms
introduced in earlier chapters but not yet examined in depth: the
\texttt{AssetHandle\textless{}T\textgreater{}} and
\texttt{ResourceHandle\textless{}T\textgreater{}} types that name assets
without owning their memory, the \texttt{SlotMap} arenas that store
entities and resources behind generational indices, and the named
budgets that Chapter 3 promised would turn silent resource exhaustion
into explicit policy decisions. This chapter makes all of those
mechanisms precise.

The problem these mechanisms solve is not performance. It is governance.
A modern game engine manages tens of thousands of resources across two
address spaces (CPU and GPU), multiple threads with different ownership
rules, and a streaming pipeline that loads and evicts data continuously.
Without a principled approach to identity, lifetime, capacity, and
observability, the engine's behavior at scale becomes an emergent
property of whichever subsystem happens to allocate last. Silent VRAM
exhaustion, stale pointer dereferences, untraceable frame-time
regressions, and test suites that pass on the developer's machine but
fail on a target platform are all symptoms of the same architectural
gap: the engine lacks a first-class vocabulary for naming resources,
limiting their consumption, observing their behavior, and verifying its
own contracts.

This chapter fills that gap. We begin with identity: why stable handles
are the right abstraction for naming resources in a system where
physical storage locations change. We then examine budgets: the named
limits and arbitration policies that transform resource exhaustion from
a crash into a decision. From there we turn to instrumentation: the
profiling, tracing, and counter infrastructure that gives every
subsystem a voice in the engine's performance story. Finally, we address
reliability: the testing, configuration, and delivery practices that
make the engine's contracts verifiable rather than aspirational.

\section*{What You Will Learn in This
Chapter}\label{what-you-will-learn-in-this-chapter-5}
\addcontentsline{toc}{section}{What You Will Learn in This Chapter}
...
\tightlist
\item
  \textbf{Identity Before Address:} Why game code should name resources
  with stable handles rather than raw pointers, and how generational
  indices and slot-map arenas provide safe, cache-friendly indirection.
\item
  \textbf{Budgets as Policy:} How named limits for GPU memory, upload
  bandwidth, and frame time transform silent failures into explicit
  arbitration decisions, with the concrete \texttt{MemoryBudget} struct
  and its eviction policies.
\item
  \textbf{Resource Lifetime and Residency:} The distinction between
  logical asset lifetime, CPU residency, and GPU residency, and how each
  layer is managed independently.
\item
  \textbf{Instrumentation as Architecture:} How profiling scopes,
  counters, and gauges form a data flow that crosses thread and domain
  boundaries, feeding Tracy, an in-engine overlay, and trace exports
  through a single API.
\item
  \textbf{Reliability at Boundaries:} How unit, integration, and
  scenario tests target different classes of engine failure, and how
  configuration, feature policy, and delivery checks close the gap
  between a working engine and a dependable product.
\end{itemize}

\section{Identity Before Address}\label{sec-identity}

Every engine manages resources: meshes, textures, materials, audio
clips, entities, render targets, and dozens of other asset types. Game
code needs to reference these resources, but the physical location of a
resource is unstable. A texture may begin in system RAM, move to a
staging buffer, then reside in device-local VRAM. An entity's component
data sits in a dense array that is compacted when other entities are
removed. A mesh LOD may be evicted from GPU memory under streaming
pressure and reloaded minutes later into a different allocation. If game
code holds a raw pointer or a direct memory address to any of these
resources, every relocation invalidates every reference, and every
invalidation is a potential crash.

The problem is older than game engines. Operating systems solved it for
file descriptors in the 1970s: a file descriptor is an integer index
into a per-process table, not a pointer to a kernel structure. Closing a
file invalidates the descriptor; opening a new file may reuse the same
index, but the kernel tracks a sequence number internally so that stale
descriptors do not accidentally operate on the wrong resource. Database
connection pools, Wayland's object IDs, X11 window handles, and GPU
driver resource handles all follow the same principle. The consumer
receives an opaque token. The owner keeps the real resource behind a
lookup table. Lifetime is managed by the owner, not by the consumer.

Game engines converged on this pattern independently, driven by the same
pressures. Unreal Engine's \texttt{FRenderResource} system assigns
opaque indices to GPU resources and uses proxy objects to decouple
game-side references from render-side memory. Unity's
\texttt{InstanceID} is a generational integer that identifies managed
objects without exposing their heap address. Godot's \texttt{RID}
(Resource ID) is an opaque handle into the \texttt{RenderingServer}'s
internal pools, giving game code a stable name for GPU resources that
the rendering backend is free to relocate or rebuild. The convergence is
not coincidental: any engine that supports asynchronous loading,
streaming, LOD transitions, or multi-threaded rendering must decouple
the identity of a resource from its physical storage.

Smallworld's resource model, introduced in Chapter 3, follows this
consensus. Game code holds opaque handles:
\texttt{AssetHandle\textless{}T\textgreater{}} for immutable shared
assets and \texttt{ResourceHandle\textless{}T\textgreater{}} for mutable
resources. Both are thin wrappers around a generational index. The
engine owns the actual data and manages its lifetime, location, and
residency. Handles are cheap to copy (they are two \texttt{u32} fields
and a zero-sized type marker), safe to store in components, safe to
serialize, and safe to hold across frames. They can outlive the resource
they name, because a generation check on access produces a clean miss
rather than undefined behavior.

The rest of this section makes the mechanism precise: what a
generational index is, how a slot map implements it, what failure modes
the design prevents, and what guarantees a handle can and cannot
provide.

\subsection{Generational Handles and Resource
Pools}\label{sec-generational-handles}

A \textbf{generational index} is a pair: a slot number that identifies a
position in a dense array, and a generation counter that distinguishes
the current occupant from all previous and future occupants of that
slot. Together, the two fields form a handle that is stable (the slot
number does not change when other slots are modified), compact (two
\texttt{u32} values: 8 bytes total), and tamper-evident (a stale
generation is detectable at the cost of a single integer comparison).

The data structure that backs generational handles is the \textbf{slot
map}, sometimes called a generational arena. A slot map is a dense array
of entries, where each entry pairs a generation counter with either live
data or a free-list link. When a new resource is inserted, the slot map
pops a slot from the free list, increments its generation, stores the
data, and returns a handle containing the slot index and the new
generation. When a resource is removed, the slot map increments the
generation again (or simply marks the slot as free while retaining the
generation), pushes the slot onto the free list, and leaves the data to
be overwritten by the next insertion. Any handle that still refers to
the old generation will find, on its next access attempt, that the
generation in the slot does not match the generation in the handle. The
access fails cleanly: the slot map returns \texttt{None}, or an error,
or a sentinel value, depending on the API. It never returns the new
occupant's data, because the new occupant has a different generation.

Chapter 3 introduced the \texttt{AssetHandle\textless{}T\textgreater{}}
and \texttt{ResourceHandle\textless{}T\textgreater{}} types and
explained the generational index pattern at a conceptual level. Here we
examine the slot map itself, starting with its core structure:

\begin{codelisting}

\caption{\label{lst-slotmap}SlotMap: generational arena with dense
storage and O(1) access}

\centering{

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{struct}\NormalTok{ SlotMap}\OperatorTok{\textless{}}\NormalTok{K}\OperatorTok{,}\NormalTok{ V}\OperatorTok{\textgreater{}} \OperatorTok{\{}
\NormalTok{    entries}\OperatorTok{:} \DataTypeTok{Vec}\OperatorTok{\textless{}}\NormalTok{Entry}\OperatorTok{\textless{}}\NormalTok{V}\OperatorTok{\textgreater{}\textgreater{},}
\NormalTok{    free\_head}\OperatorTok{:} \DataTypeTok{Option}\OperatorTok{\textless{}}\DataTypeTok{u32}\OperatorTok{\textgreater{},}
\NormalTok{    len}\OperatorTok{:} \DataTypeTok{u32}\OperatorTok{,}
\OperatorTok{\}}

\KeywordTok{struct}\NormalTok{ Entry}\OperatorTok{\textless{}}\NormalTok{V}\OperatorTok{\textgreater{}} \OperatorTok{\{}
\NormalTok{    generation}\OperatorTok{:} \DataTypeTok{u32}\OperatorTok{,}
\NormalTok{    value}\OperatorTok{:} \DataTypeTok{Option}\OperatorTok{\textless{}}\NormalTok{V}\OperatorTok{\textgreater{},}
\NormalTok{    next\_free}\OperatorTok{:} \DataTypeTok{Option}\OperatorTok{\textless{}}\DataTypeTok{u32}\OperatorTok{\textgreater{},}
\OperatorTok{\}}

\KeywordTok{impl}\OperatorTok{\textless{}}\NormalTok{K}\OperatorTok{:}\NormalTok{ HandleKey}\OperatorTok{,}\NormalTok{ V}\OperatorTok{\textgreater{}}\NormalTok{ SlotMap}\OperatorTok{\textless{}}\NormalTok{K}\OperatorTok{,}\NormalTok{ V}\OperatorTok{\textgreater{}} \OperatorTok{\{}
    \KeywordTok{fn}\NormalTok{ insert(}\OperatorTok{\&}\KeywordTok{mut} \KeywordTok{self}\OperatorTok{,}\NormalTok{ value}\OperatorTok{:}\NormalTok{ V) }\OperatorTok{{-}\textgreater{}}\NormalTok{ K }\OperatorTok{\{}
        \KeywordTok{let}\NormalTok{ index }\OperatorTok{=} \ControlFlowTok{match} \KeywordTok{self}\OperatorTok{.}\NormalTok{free\_head }\OperatorTok{\{}
            \ConstantTok{Some}\NormalTok{(i) }\OperatorTok{=\textgreater{}} \OperatorTok{\{}
                \KeywordTok{self}\OperatorTok{.}\NormalTok{free\_head }\OperatorTok{=} \KeywordTok{self}\OperatorTok{.}\NormalTok{entries[i }\KeywordTok{as} \DataTypeTok{usize}\NormalTok{]}\OperatorTok{.}\NormalTok{next\_free}\OperatorTok{;}
\NormalTok{                i}
            \OperatorTok{\}}
            \ConstantTok{None} \OperatorTok{=\textgreater{}} \OperatorTok{\{}
                \KeywordTok{self}\OperatorTok{.}\NormalTok{entries}\OperatorTok{.}\NormalTok{push(Entry }\OperatorTok{\{}
\NormalTok{                    generation}\OperatorTok{:} \DecValTok{0}\OperatorTok{,}
\NormalTok{                    value}\OperatorTok{:} \ConstantTok{None}\OperatorTok{,}
\NormalTok{                    next\_free}\OperatorTok{:} \ConstantTok{None}\OperatorTok{,}
                \OperatorTok{\}}\NormalTok{)}\OperatorTok{;}
\NormalTok{                (}\KeywordTok{self}\OperatorTok{.}\NormalTok{entries}\OperatorTok{.}\NormalTok{len() }\OperatorTok{{-}} \DecValTok{1}\NormalTok{) }\KeywordTok{as} \DataTypeTok{u32}
            \OperatorTok{\}}
        \OperatorTok{\};}
        \KeywordTok{let}\NormalTok{ entry }\OperatorTok{=} \OperatorTok{\&}\KeywordTok{mut} \KeywordTok{self}\OperatorTok{.}\NormalTok{entries[index }\KeywordTok{as} \DataTypeTok{usize}\NormalTok{]}\OperatorTok{;}
\NormalTok{        entry}\OperatorTok{.}\NormalTok{generation }\OperatorTok{+=} \DecValTok{1}\OperatorTok{;}
\NormalTok{        entry}\OperatorTok{.}\NormalTok{value }\OperatorTok{=} \ConstantTok{Some}\NormalTok{(value)}\OperatorTok{;}
        \KeywordTok{self}\OperatorTok{.}\NormalTok{len }\OperatorTok{+=} \DecValTok{1}\OperatorTok{;}
        \PreprocessorTok{K::}\NormalTok{from\_raw(index}\OperatorTok{,}\NormalTok{ entry}\OperatorTok{.}\NormalTok{generation)}
    \OperatorTok{\}}

    \KeywordTok{fn}\NormalTok{ remove(}\OperatorTok{\&}\KeywordTok{mut} \KeywordTok{self}\OperatorTok{,}\NormalTok{ key}\OperatorTok{:}\NormalTok{ K) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{Option}\OperatorTok{\textless{}}\NormalTok{V}\OperatorTok{\textgreater{}} \OperatorTok{\{}
        \KeywordTok{let}\NormalTok{ (index}\OperatorTok{,}\NormalTok{ gen) }\OperatorTok{=}\NormalTok{ key}\OperatorTok{.}\NormalTok{into\_raw()}\OperatorTok{;}
        \KeywordTok{let}\NormalTok{ entry }\OperatorTok{=} \OperatorTok{\&}\KeywordTok{mut} \KeywordTok{self}\OperatorTok{.}\NormalTok{entries[index }\KeywordTok{as} \DataTypeTok{usize}\NormalTok{]}\OperatorTok{;}
        \ControlFlowTok{if}\NormalTok{ entry}\OperatorTok{.}\NormalTok{generation }\OperatorTok{!=}\NormalTok{ gen }\OperatorTok{||}\NormalTok{ entry}\OperatorTok{.}\NormalTok{value}\OperatorTok{.}\NormalTok{is\_none() }\OperatorTok{\{}
            \ControlFlowTok{return} \ConstantTok{None}\OperatorTok{;}
        \OperatorTok{\}}
        \KeywordTok{let}\NormalTok{ value }\OperatorTok{=}\NormalTok{ entry}\OperatorTok{.}\NormalTok{value}\OperatorTok{.}\NormalTok{take()}\OperatorTok{;}
\NormalTok{        entry}\OperatorTok{.}\NormalTok{next\_free }\OperatorTok{=} \KeywordTok{self}\OperatorTok{.}\NormalTok{free\_head}\OperatorTok{;}
        \KeywordTok{self}\OperatorTok{.}\NormalTok{free\_head }\OperatorTok{=} \ConstantTok{Some}\NormalTok{(index)}\OperatorTok{;}
        \KeywordTok{self}\OperatorTok{.}\NormalTok{len }\OperatorTok{{-}=} \DecValTok{1}\OperatorTok{;}
\NormalTok{        value}
    \OperatorTok{\}}

    \KeywordTok{fn}\NormalTok{ get(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ key}\OperatorTok{:}\NormalTok{ K) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{Option}\OperatorTok{\textless{}\&}\NormalTok{V}\OperatorTok{\textgreater{}} \OperatorTok{\{}
        \KeywordTok{let}\NormalTok{ (index}\OperatorTok{,}\NormalTok{ gen) }\OperatorTok{=}\NormalTok{ key}\OperatorTok{.}\NormalTok{into\_raw()}\OperatorTok{;}
        \KeywordTok{let}\NormalTok{ entry }\OperatorTok{=} \OperatorTok{\&}\KeywordTok{self}\OperatorTok{.}\NormalTok{entries[index }\KeywordTok{as} \DataTypeTok{usize}\NormalTok{]}\OperatorTok{;}
        \ControlFlowTok{if}\NormalTok{ entry}\OperatorTok{.}\NormalTok{generation }\OperatorTok{==}\NormalTok{ gen }\OperatorTok{\{}
\NormalTok{            entry}\OperatorTok{.}\NormalTok{value}\OperatorTok{.}\NormalTok{as\_ref()}
...
The \texttt{HandleKey} trait abstracts over handle types so that the
same slot map implementation serves entities (\texttt{EntityId}), GPU
resources (\texttt{GpuId}), assets
(\texttt{AssetHandle\textless{}T\textgreater{}}), and any other handle
type the engine needs. Each handle type is a newtype around the same
\texttt{(index,\ generation)} pair, but the type system prevents a mesh
handle from being used to look up a texture, even though the underlying
representation is identical. This is a deliberate use of Rust's
zero-cost newtype pattern: the
\texttt{PhantomData\textless{}T\textgreater{}} marker on
\texttt{AssetHandle\textless{}T\textgreater{}} provides type safety at
compile time without adding any runtime cost.

Listing~\ref{lst-slotmap} shows the core operations. \texttt{insert}
pops a free slot (or grows the array), bumps the generation, stores the
value, and returns a handle. \texttt{remove} checks the generation,
takes the value, and pushes the slot back onto the free list.
\texttt{get} checks the generation and returns a reference only if it
matches. Every operation is O(1). The lookup is a bounds-checked array
index followed by an integer comparison, which modern CPUs execute in a
handful of cycles with no cache misses beyond the initial array access.
This is why slot maps are preferred over hash maps for resource pools in
performance-critical engines: a hash map must compute a hash, probe a
bucket chain, and compare keys; a slot map does an array index and an
equality check.

The dense storage also matters for iteration. When a system needs to
iterate over all live resources in a pool (for example, the render
thread iterating over all GPU meshes to rebuild the TLAS), a slot map's
backing array provides cache-friendly sequential access. Entries that
have been freed leave gaps, but the gaps are bounded by the pool's
high-water mark, and the generation check skips them cheaply. For pools
where iteration density is critical, a secondary dense array of live
indices can eliminate gaps entirely, at the cost of maintaining the
secondary array on insert and remove. Smallworld's GPU resource pools
use this pattern: the
\texttt{SlotMap\textless{}GpuId,\ GpuMesh\textgreater{}} holds the data,
and a parallel dense \texttt{Vec\textless{}GpuId\textgreater{}} holds
the keys of live entries for tight iteration during culling and draw
submission.

Figure \ref{fig:generational-handle} illustrates the generation check in
action. A current handle with a matching generation resolves to live
data. A stale handle whose generation does not match the slot's current
generation produces a clean miss. The slot may now hold a completely
different resource, but the stale handle never sees it.

\input{figures/generational-handle.tex}

\subsubsection{Handle Failure Modes}\label{sec-handle-failure}

The generational index design prevents the most dangerous failure mode
in resource management: \textbf{use-after-free}. In a raw-pointer
system, freeing a resource and then accessing it through a dangling
pointer produces undefined behavior. The program may crash, may silently
read garbage data, or may corrupt unrelated memory. The failure is
non-deterministic and often manifests far from the bug's origin, making
it one of the hardest classes of defect to diagnose. In a generational
handle system, the equivalent scenario produces a defined, detectable
result: the handle's generation does not match the slot's generation,
the access returns \texttt{None}, and the caller can handle the miss
explicitly.

This guarantee holds even when slots are reused. Consider the following
sequence: a texture is allocated in slot 5 with generation 1. A
component stores the handle \texttt{(index=5,\ generation=1)}. The
texture is freed; slot 5 returns to the free list with generation 1.
Later, a new mesh is allocated in slot 5, bumping the generation to 2.
The stale handle \texttt{(index=5,\ generation=1)} now points at a slot
whose generation is 2. The access fails cleanly: the caller gets
\texttt{None}, not a mesh masquerading as a texture. The generation
counter has broken the ABA problem, the classic concurrent-programming
hazard where a memory location is reused for a different value and a
stale reference cannot distinguish the new value from the old one.

There are several failure modes that the generational handle design
addresses, and a few that it deliberately does not:

\textbf{Stale handles after despawn.} When an entity is despawned, its
\texttt{EntityId} handle becomes stale. Any system, behavior, or
component that cached the handle will get a clean miss on the next
access. This is the primary defense against a common class of gameplay
bugs: a projectile targets an enemy, the enemy despawns, and the
projectile's update function tries to read the enemy's transform. With a
raw pointer, this is a crash. With a generational handle,
\texttt{world.get(target)} returns \texttt{None}, and the projectile can
respond appropriately (explode at last known position, despawn itself,
or simply miss).

\textbf{Double free.} Calling \texttt{remove} with a handle that has
already been removed is a no-op: the generation has already been bumped
by the first removal, so the second call's generation check fails. The
caller can detect this (the return value is \texttt{None}), but even if
it does not check, no state is corrupted.

\textbf{Cross-type confusion.} Because each handle type
(\texttt{AssetHandle\textless{}MeshAsset\textgreater{}},
\texttt{AssetHandle\textless{}TextureAsset\textgreater{}},
\texttt{GpuId}, \texttt{EntityId}) is a distinct Rust type, the compiler
prevents using a mesh handle to look up a texture at compile time. This
is a stronger guarantee than the runtime generation check: it catches an
entire category of bugs before the program runs.

\textbf{Generation overflow.} A \texttt{u32} generation counter can wrap
around after approximately 4.3 billion increments per slot. In practice,
this requires a single slot to be allocated and freed billions of times,
which is not a realistic workload. If an engine needed to guard against
this (for example, in a long-running server), the mitigation is
straightforward: use a \texttt{u64} generation, or reserve generation 0
as a sentinel and skip it on wrap. Smallworld uses \texttt{u32}
generations, which are sufficient for any practical session length.

\textbf{Concurrent access races.} Generational handles do not provide
thread-safety by themselves. A \texttt{SlotMap} is an ordinary mutable
data structure; concurrent \texttt{insert} and \texttt{get} from
different threads without synchronization is a data race. Smallworld's
threading model, described in Chapter 3, prevents this structurally:
each slot map is owned by a single thread. The entity slot map lives on
the Game Thread. The GPU resource pools live on the Render Thread.
Handles cross thread boundaries freely (they are \texttt{Send\ +\ Sync}
by construction: two \texttt{u32} fields and a zero-size marker), but
the slot maps they index do not. This is the thread-ownership principle
applied to resource pools: the handle is a message; the pool is owned
state.

\textbf{What handles cannot promise.} A handle does not promise that the
resource is \emph{ready}. An
\texttt{AssetHandle\textless{}TextureAsset\textgreater{}} may refer to a
texture that has been requested but not yet loaded, or that has been
loaded into CPU memory but not yet uploaded to the GPU. The handle's
generation check confirms only that the resource \emph{exists} in the
slot map (it has been allocated and not yet freed). Readiness is a
separate query, exposed through the asset system's loading-state API.
This separation is deliberate: collapsing identity and readiness into a
single check would force every handle access to also check loading
state, penalizing the common case (the resource is already loaded) to
handle the uncommon case (the resource is still in flight). Chapter 7
examines the loading-state model in detail.

\section{Budgets Turn Failure into Policy}\label{sec-budgets}

Chapter 3 introduced the budget-explicit principle with a failure-mode
story: a level designer adds unique textures until VRAM is silently
exhausted on a target GPU, frame times spike, and the team spends days
diagnosing a problem that no system reported. The principle states that
\textbf{frame time, GPU memory, upload bandwidth, and streaming distance
are explicit budgets with engine arbitration, not emergent properties}.
Chapter 3 promised that this chapter would present the concrete types
and arbitration policies. Here we deliver on that promise.

The core insight is that a budget is not a limit. A limit is a number
that, when exceeded, causes a failure. A budget is a number that, when
approached, causes a \emph{decision}. The decision might be to evict the
least-recently-used texture, to lower a distant entity's LOD, to
throttle streaming IO to preserve frame-time headroom, or to reject an
allocation outright and use a fallback. The critical difference is that
the decision is explicit, logged, and observable. The engine does not
silently degrade; it makes a traceable choice.

This is not a novel idea. Every engine that ships on consoles enforces
similar limits, because consoles have fixed memory and the certification
process requires that the game never exceed it. The Xbox certification
requirements include specific tests for memory stability under sustained
load. PlayStation's Technical Requirements Checklist (TRC) mandates that
games handle memory exhaustion gracefully. Mobile platforms are even
more constrained: iOS will terminate a foreground app that exceeds its
memory allocation without warning, making budget enforcement a survival
requirement, not a quality-of-life feature. What varies across engines
is whether these budgets are architectural (designed into every pool
from the start) or retrofitted (bolted on after the first out-of-memory
crash in QA). Smallworld makes them architectural.

\subsection{The MemoryBudget Struct}\label{the-memorybudget-struct}

Every resource pool in the engine carries a \texttt{MemoryBudget}: a
struct that records the pool's byte limit, current usage, and high-water
mark. The struct is deliberately simple, because its value comes not
from cleverness but from ubiquity: every pool has one, every allocation
checks it, and every budget publishes its state as an instrumentation
gauge.

\begin{codelisting}

\caption{\label{lst-memory-budget}MemoryBudget: the universal resource
accounting struct}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{struct}\NormalTok{ MemoryBudget }\OperatorTok{\{}
\NormalTok{    limit\_bytes}\OperatorTok{:}      \DataTypeTok{u64}\OperatorTok{,}
\NormalTok{    used\_bytes}\OperatorTok{:}       \DataTypeTok{u64}\OperatorTok{,}
\NormalTok{    high\_water\_bytes}\OperatorTok{:} \DataTypeTok{u64}\OperatorTok{,}
\OperatorTok{\}}

\KeywordTok{impl}\NormalTok{ MemoryBudget }\OperatorTok{\{}
    \KeywordTok{fn}\NormalTok{ can\_allocate(}\OperatorTok{\&}\KeywordTok{self}\OperatorTok{,}\NormalTok{ bytes}\OperatorTok{:} \DataTypeTok{u64}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{bool} \OperatorTok{\{}
        \KeywordTok{self}\OperatorTok{.}\NormalTok{used\_bytes }\OperatorTok{+}\NormalTok{ bytes }\OperatorTok{\textless{}=} \KeywordTok{self}\OperatorTok{.}\NormalTok{limit\_bytes}
    \OperatorTok{\}}

    \KeywordTok{fn}\NormalTok{ allocate(}\OperatorTok{\&}\KeywordTok{mut} \KeywordTok{self}\OperatorTok{,}\NormalTok{ bytes}\OperatorTok{:} \DataTypeTok{u64}\NormalTok{) }\OperatorTok{\{}
        \KeywordTok{self}\OperatorTok{.}\NormalTok{used\_bytes }\OperatorTok{+=}\NormalTok{ bytes}\OperatorTok{;}
        \ControlFlowTok{if} \KeywordTok{self}\OperatorTok{.}\NormalTok{used\_bytes }\OperatorTok{\textgreater{}} \KeywordTok{self}\OperatorTok{.}\NormalTok{high\_water\_bytes }\OperatorTok{\{}
            \KeywordTok{self}\OperatorTok{.}\NormalTok{high\_water\_bytes }\OperatorTok{=} \KeywordTok{self}\OperatorTok{.}\NormalTok{used\_bytes}\OperatorTok{;}
        \OperatorTok{\}}
    \OperatorTok{\}}

    \KeywordTok{fn}\NormalTok{ free(}\OperatorTok{\&}\KeywordTok{mut} \KeywordTok{self}\OperatorTok{,}\NormalTok{ bytes}\OperatorTok{:} \DataTypeTok{u64}\NormalTok{) }\OperatorTok{\{}
        \KeywordTok{self}\OperatorTok{.}\NormalTok{used\_bytes }\OperatorTok{{-}=}\NormalTok{ bytes}\OperatorTok{;}
    \OperatorTok{\}}

    \KeywordTok{fn}\NormalTok{ pressure(}\OperatorTok{\&}\KeywordTok{self}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{f32} \OperatorTok{\{}
        \KeywordTok{self}\OperatorTok{.}\NormalTok{used\_bytes }\KeywordTok{as} \DataTypeTok{f32} \OperatorTok{/} \KeywordTok{self}\OperatorTok{.}\NormalTok{limit\_bytes }\KeywordTok{as} \DataTypeTok{f32}
    \OperatorTok{\}}

    \KeywordTok{fn}\NormalTok{ headroom(}\OperatorTok{\&}\KeywordTok{self}\NormalTok{) }\OperatorTok{{-}\textgreater{}} \DataTypeTok{u64} \OperatorTok{\{}
        \KeywordTok{self}\OperatorTok{.}\NormalTok{limit\_bytes}\OperatorTok{.}\NormalTok{saturating\_sub(}\KeywordTok{self}\OperatorTok{.}\NormalTok{used\_bytes)}
    \OperatorTok{\}}
\OperatorTok{\}}
\end{Highlighting}
\end{Shaded}
...
Listing~\ref{lst-memory-budget} shows the struct and its core
operations. \texttt{can\_allocate} checks whether a proposed allocation
would exceed the limit. \texttt{allocate} increments usage and updates
the high-water mark. \texttt{free} decrements usage. \texttt{pressure}
returns the fraction of the budget currently consumed (0.0 to 1.0),
which the arbitration logic and the instrumentation overlay both read.
\texttt{headroom} returns the remaining bytes before the limit, used by
streaming and upload scheduling to size their next batch.

The high-water mark deserves attention. It records the maximum
\texttt{used\_bytes} observed since the budget was created (or since the
mark was manually reset). This value serves two purposes. First, it
gives profiling tools a measure of peak demand, which is more
informative than current usage for capacity planning. A pool that
averages 60\% utilization but peaks at 98\% is one asset away from
eviction pressure; a pool that averages 60\% and peaks at 62\% has
comfortable headroom. Second, it provides a regression signal: if the
high-water mark for the mesh pool increases by 30\% between builds,
something changed that is worth investigating, even if the pool never
exceeded its limit.

Every GPU pool in the architecture specification carries a budget field.
The \texttt{GpuMeshPool}, \texttt{GpuTexturePool}, and staging pool each
embed a \texttt{MemoryBudget}. When the engine initializes, it queries
the GPU's available memory through \texttt{wgpu}'s adapter info, applies
platform-specific scaling factors (a shipping console build may reserve
a known fraction for the OS and system services), and distributes the
total across pools according to a configuration table. The configuration
is data, not code: a game can ship different budget profiles for
different hardware tiers without recompiling the engine. The capability
tier system (described in the architecture specification) sets the
defaults; the game's configuration overrides them.

\subsection{Budget Arbitration}\label{budget-arbitration}

A budget without arbitration is just a counter. The value of the
\texttt{MemoryBudget} struct comes from the decisions it enables. When a
pool reaches its limit, the engine must decide what to do. The decision
depends on the pool's role and the resource's importance. Smallworld
defines three arbitration responses, applied in order of preference:

\textbf{Evict LRU.} The least-recently-used resource in the pool is
evicted to make room for the new allocation. This is the default
response for GPU resource pools (meshes, textures) under streaming
workloads. The streaming system continuously loads and evicts resources
as the camera moves; LRU eviction ensures that resources the player is
looking at stay resident while resources behind the camera or beyond the
streaming radius are released. The LRU timestamp is updated on every
access (not just on every frame), so a texture that is bound for
rendering every frame stays fresh, while a texture that was loaded
speculatively but never rendered ages naturally toward eviction.

\textbf{Lower LOD.} When eviction alone is insufficient (the pool is
full and every resident resource is actively in use), the arbiter can
lower the LOD of distant resources. A high-resolution texture that
serves a distant object can be replaced by a lower-resolution mip,
freeing a significant fraction of the allocation while preserving visual
coverage. This is the response that keeps the scene visually complete
under memory pressure: the player sees blurrier textures in the
distance, not missing geometry. The dynamic resolution scaling (DRS)
controller, described in the rendering chapters, makes the complementary
decision for render targets: if the frame-time budget is under pressure,
it lowers the internal rendering resolution. Both are explicit, logged,
observable tradeoffs.

\textbf{Reject.} When neither eviction nor LOD reduction can free enough
space (for example, a budget-protected resource like the current
weapon's textures cannot be evicted or reduced), the allocation is
rejected. The requesting system receives a failure signal and must use
its fallback: a placeholder texture, a simpler mesh, or a deferred
retry. Rejection is the least desirable outcome, but it is infinitely
preferable to the alternative: silently exceeding the budget, causing
driver-level memory pressure, and producing unpredictable performance
degradation that no engine system reports.

Figure \ref{fig:budget-arbitration} illustrates the arbitration flow.
Named budgets publish their state to a central arbiter, which selects
the appropriate response and logs the decision through the
instrumentation system. The same gauges that drive arbitration decisions
are displayed in the profiling overlay, closing the loop between
allocation behavior and developer visibility.

\input{figures/budget-arbitration.tex}

The arbitration logic itself is straightforward. The complexity is in
making it ubiquitous. Every pool must check its budget before
allocating. Every eviction must free the resource cleanly (including any
GPU-side cleanup that requires the render thread). Every rejection must
be handled by the caller. And every decision must be logged so that a
developer investigating a visual artifact or a performance regression
can trace it to a specific budget event at a specific frame. This is why
the budget system is architecture, not optimization: it touches every
resource pool in the engine, and it must be designed in from the start.
Retrofitting budgets onto pools that were built without them requires
auditing every allocation site and every consumer of every pool, a task
that grows superlinearly with the size of the codebase.

The budget arbiter is not a global singleton. Each pool owns its own
\texttt{MemoryBudget} and makes its own eviction decisions. The
``arbiter'' is the collection of budget-aware logic distributed across
pools, unified by the shared \texttt{MemoryBudget} type and the shared
instrumentation gauges. This design avoids the centralization trap: a
global arbiter that must understand every pool's eviction semantics
becomes a bottleneck for both execution (it serializes allocation
decisions across threads) and development (every new pool type requires
changes to the central arbiter). Distributed arbitration with a shared
vocabulary is the pattern that scales.

\subsection{Resource Lifetime and
Residency}\label{sec-lifetime-residency}

A resource in a game engine does not have a single lifetime. It has at
least three, each managed by a different subsystem:

\textbf{Logical lifetime.} The duration for which a handle exists and
can be resolved. A game system spawns an entity and receives an
\texttt{EntityId}. It stores that ID in a target list, passes it to a
behavior, or serializes it into a save file. The logical lifetime begins
at spawn and ends at despawn. During this window, the handle is valid:
\texttt{world.get(id)} returns the entity's data.

\textbf{CPU residency.} The duration for which the asset's decoded data
occupies system RAM. A mesh's vertex data, a texture's pixel data, an
audio clip's sample data: these consume memory and must be loaded from
disk (or generated) before they can be used. CPU residency begins when
the asset pipeline completes decoding and ends when the data is evicted
under memory pressure. Crucially, CPU residency can end while the
logical lifetime continues: the game still holds a valid
\texttt{AssetHandle}, but the data behind it has been evicted. The next
access triggers a reload.

\textbf{GPU residency.} The duration for which the resource occupies
device-local GPU memory. A texture's GPU-resident copy, a mesh's vertex
buffer, a material's uniform data. GPU residency begins when the render
thread uploads the data from the staging pool into device-local memory
and ends when the GPU pool evicts it. GPU residency can end
independently of both CPU residency and logical lifetime: the GPU pool
may evict a distant texture to make room for a closer one, while the
CPU-side data remains resident for fast re-upload if the camera returns.

Figure \ref{fig:resource-lifetime-layers} illustrates the three layers.
The key insight is that each layer is independently managed. Logical
lifetime is determined by game logic (spawn and despawn). CPU residency
is determined by the asset server and its memory budget. GPU residency
is determined by the render thread's resource pools and their budgets.
The three layers communicate through handles and loading states, but
they do not share lifetime: evicting a resource from GPU memory does not
despawn the entity that references it, and despawning an entity does not
immediately free the GPU memory its mesh occupied.

\input{figures/resource-lifetime-layers.tex}

This independence is what makes the architecture specification's dictum
precise: \textbf{zero references makes an asset evictable, not evicted.}
When the last entity holding an
\texttt{AssetHandle\textless{}TextureAsset\textgreater{}} is despawned,
the texture's reference count drops to zero. The asset is now eligible
for eviction, but eviction happens only when the budget arbiter needs
the space. If the player re-enters the area before memory pressure
triggers eviction, the texture is still resident, and the re-spawned
entities can use it immediately without a load. This is the caching
behavior that every virtual memory system provides: recently-freed pages
remain in the page cache until pressure forces reclamation. The engine's
asset system applies the same principle to game resources.

The streaming system, detailed in Chapter 11, adds a fourth layer of
complexity: \textbf{streaming residency}, where data is loaded at
varying quality levels (mip levels for textures, LOD tiers for meshes,
brick resolutions for voxel volumes) and the budget arbiter continuously
adjusts which quality level is resident based on distance, priority, and
available budget. The residency invariant from the architecture
specification ensures that this process is always safe: the coarsest
tier of everything is pinned resident, so every possible residency miss
has a rendering answer (fall back to a coarser level), and the failure
mode under any pressure is blur, never holes, never stalls.

\subsubsection{Memory for Rust Engines}\label{memory-for-rust-engines}

Rust's ownership system changes the shape of memory errors without
eliminating the need for capacity planning. In a C++ engine, a
use-after-free in a resource pool is a memory-safety bug: the program
reads deallocated memory and produces undefined behavior. In a Rust
engine, the borrow checker prevents this class of bug at compile time:
you cannot hold a reference to the interior of a \texttt{Vec} that
another system might reallocate, because the compiler tracks the
lifetime of every borrow. The generational handle pattern is not
primarily a safety mechanism in Rust (the compiler already prevents
dangling references); it is an \emph{identity} mechanism that decouples
names from storage locations, enabling relocation, serialization, and
cross-thread communication.

What Rust does not prevent is capacity exhaustion. The compiler cannot
know at build time how much GPU memory the game will consume at runtime,
or whether the streaming system's demand will exceed the upload budget
on a slow disk. These are runtime problems that require runtime
infrastructure: the \texttt{MemoryBudget} struct, the arbitration
policies, and the instrumentation gauges described in this chapter.
Rust's type system ensures that the bookkeeping is correct (every
allocation through the pool updates the budget, because the API does not
expose an unbudgeted allocation path). But the policy, the decisions
about what to evict and when, is engineering judgment encoded in data,
not a property that any type system can derive.

The practical consequence for engine developers is that Rust eliminates
the low-level memory-safety bugs (dangling pointers, use-after-free,
data races) that consume enormous debugging effort in C++ engines,
freeing that effort for the higher-level capacity problems (budget
tuning, eviction policy, streaming priorities) that determine how the
game performs on real hardware. This is a genuine productivity
advantage, but it is not a substitute for the architectural
infrastructure this chapter describes. A Rust engine without budgets
will run out of GPU memory just as surely as a C++ engine without
budgets; it will simply not crash while doing so.

\section{Instrumentation Is Part of the
Architecture}\label{sec-instrumentation}

A budget is only as useful as the evidence behind it. The
\texttt{MemoryBudget} struct records usage and headroom, but the
developer investigating a frame-time regression needs more than a
current value: they need a timeline of when each pool's pressure
changed, which system was responsible for the spike, how long the spike
lasted, and whether it correlated with a streaming event, a physics
solve, or a render pass. This is the job of the instrumentation system:
to provide structured, cross-domain, cross-thread evidence that budget
decisions, performance characteristics, and failure modes are observable
rather than opaque.

The game engine industry arrived at this conclusion through decades of
painful experience. Early engines had no built-in profiling; developers
relied on external tools like VTune, PIX, or Instruments to capture CPU
and GPU traces. These tools are powerful but external: they cannot see
the engine's semantic structure (which system was running, which pass
was executing, which entity was being processed), only the raw
instruction stream. A 4 ms spike in the ``cull'' function is visible in
VTune, but VTune cannot tell you that the spike happened because the
streaming system loaded 200 new entities this frame and the cull loop
had to process them all before the render thread could begin recording.

The solution, adopted by every major engine, is to embed instrumentation
markers in the engine's own code. Unreal Engine's
\texttt{SCOPED\_NAMED\_EVENT} and \texttt{stat} system annotate CPU work
with hierarchical scopes that appear in Unreal Insights, Epic's
profiling tool. Unity's \texttt{ProfilerMarker} and
\texttt{Profiler.BeginSample} / \texttt{Profiler.EndSample} provide the
same capability for Unity's Profiler window. Godot 4.3+ instruments its
rendering pipeline with \texttt{RenderingDevice::push\_debug\_group},
visible in RenderDoc and its own performance monitor. The pattern is
universal: engine code annotates its own structure, and profiling tools
consume those annotations to present a semantically meaningful timeline.

Smallworld follows this consensus with a deliberate design choice:
\textbf{one instrumentation API, multiple sinks}. The engine defines a
small set of macros that annotate code structure (scoped timers,
counters, gauges). These macros produce data that flows to multiple
consumers simultaneously: Tracy for deep offline analysis, an in-engine
\texttt{egui} overlay for live monitoring during development, and a
chrome-trace JSON export for CI integration and offline diffing. The
engine never builds its own deep profiler; it builds the annotation
layer and the transport, and it delegates deep analysis to tools that
already do it well.

\subsection{The Instrumentation API}\label{the-instrumentation-api}

The instrumentation API is three macros:

\begin{codelisting}

\caption{\label{lst-instrumentation-api}The instrumentation API: scopes,
counters, and gauges}
...
\begin{Shaded}
\begin{Highlighting}[]
\PreprocessorTok{profile\_scope!}\NormalTok{(}\StringTok{"cull.shadow\_views"}\NormalTok{)}\OperatorTok{;}    \CommentTok{// scoped hierarchical timer}
\PreprocessorTok{counter!}\NormalTok{(DRAW\_CALLS}\OperatorTok{,}\NormalTok{ n)}\OperatorTok{;}                \CommentTok{// per{-}frame counter}
\PreprocessorTok{gauge!}\NormalTok{(STREAMING\_QUEUE\_DEPTH}\OperatorTok{,}\NormalTok{ depth)}\OperatorTok{;}   \CommentTok{// sampled value}
\end{Highlighting}
\end{Shaded}
...
Listing~\ref{lst-instrumentation-api} shows the three primitives.
\texttt{profile\_scope!} creates a named scope that measures the
wall-clock time of the enclosing block. The scope is hierarchical:
\texttt{"cull.shadow\_views"} nests under \texttt{"cull"}, which nests
under the top-level frame scope. When the scope exits (via Rust's
\texttt{Drop} trait on the scope guard), the elapsed time is recorded.
\texttt{counter!} increments a named integer counter for the current
frame: draw calls submitted, entities culled, bytes uploaded. The
counter resets to zero at the start of each frame. \texttt{gauge!} sets
a named value that represents a current measurement rather than a
per-frame accumulation: queue depth, pool pressure, resident brick
count.

The \textbf{zero-cost rule} governs these macros: in shipping builds,
all three compile to no-ops. The compiler eliminates the scope guards,
counter increments, and gauge writes entirely. There is no conditional
check, no branch, no function call: the macros expand to nothing. In
development builds, the overhead per scope is nanoseconds (a
\texttt{QueryPerformanceCounter} or \texttt{clock\_gettime} call on
scope entry and exit, plus a write to a thread-local ring buffer).
Counters and gauges are atomic increments or stores to pre-allocated
slots. The total per-frame overhead for instrumentation in a development
build is typically under 100 microseconds, well within the noise floor
of a 16 ms frame.

Every execution context in the engine registers a \textbf{named thread
lane}: Game, Render, game-pool workers, render-pool workers, Streaming
Coordinator, Audio, and IO pool. When a scope fires on a thread, the
instrumentation system attributes it to that thread's lane. This is what
allows Tracy and the overlay to show a per-thread timeline: the Game
Thread's scopes appear on one row, the Render Thread's on another,
worker pool tasks appear as sub-lanes, and the developer can see at a
glance which thread is the bottleneck.

Games use the identical macros. A game system that instruments its own
logic with \texttt{profile\_scope!("ai.pathfinding")} appears in the
same timeline, on the same thread lane, as the engine's
\texttt{profile\_scope!("update.physics")}. There is no separate game
instrumentation API, no game-specific profiler, no special registration
step. The engine's macros are public, and game code uses them directly.
This ensures that a game developer investigating a frame-time spike sees
the full picture: engine work and game work on the same timeline, with
the same tools.

\subsection{Sinks}\label{sinks}

The instrumentation data flows to three sinks, each serving a different
use case:

\textbf{Tracy} is the deep-dive tool. Tracy is an open-source frame
profiler with nanosecond resolution, GPU timeline support, memory
allocation tracking, lock contention visualization, and capture-file
export. It runs as a separate application that connects to the game
process over a local socket. When Tracy is attached, the instrumentation
macros write scope, counter, and gauge data to Tracy's ring buffer using
its C API (wrapped in a thin Rust binding). Tracy renders the data as a
hierarchical timeline with per-thread lanes, GPU work interleaved with
CPU work, and the ability to inspect individual scopes, their call
counts, and their distribution across frames.

The choice to use Tracy rather than building a custom profiler is
deliberate and consequential. Tracy represents over a decade of profiler
engineering by Bartosz Taudul, with features (memory allocation
tracking, lock contention, zone comparison, statistics, frame images)
that would take years to replicate. Building a custom profiler is a
common trap for engine teams: the MVP is achievable in a few weeks, but
the depth required for real-world debugging (handling millions of scopes
per frame, correlating CPU and GPU timelines, managing multi-gigabyte
capture files) is a multi-year investment that diverts effort from the
engine itself. The correct strategy is to build the annotation layer
(the macros) and the transport (the thread-lane registration and ring
buffer), and to delegate deep analysis to a tool that already excels at
it.

\textbf{The egui overlay} is the always-available live monitor. It
renders directly in the engine's window as an \texttt{egui} panel,
toggled by a debug key. The overlay shows a rolling frame-time graph,
per-thread millisecond breakdowns, the top-N most expensive scopes, all
active counters, and the \textbf{budget table}: every registered
\texttt{MemoryBudget} with its limit, current usage, pressure
percentage, and high-water mark. The overlay is available in any
development build, on any machine, without Tracy or any external tool.
It is the first thing a developer checks when something feels slow, and
it is the tool that an artist or designer can read without profiling
expertise.

The budget table in the overlay deserves emphasis. Every
\texttt{MemoryBudget} in the engine registers itself with a
\texttt{BudgetRegistry} at creation time, publishing its limit, usage,
and pressure as instrumentation gauges. The overlay reads whatever is
registered and renders it. This is a structural guarantee: adding a new
budgeted pool without registering it with the \texttt{BudgetRegistry} is
not possible through the public API, because the \texttt{MemoryBudget}
constructor takes a registry reference and registers itself. Budget
receipts are not optional; they are a consequence of using the budget
type.

\textbf{Chrome-trace JSON export} is the offline and CI tool. On demand,
the engine writes a JSON file in Chrome's trace-event format (the format
consumed by \texttt{chrome://tracing} and Perfetto). This file can be
committed to a CI artifact store, diffed between builds, and attached to
bug reports. For CI, the export enables automated performance regression
detection: a build that increases the p99 frame time by 10\% relative to
the previous build's trace can be flagged automatically. For bug
reports, the export provides a complete, self-contained record of the
engine's behavior during the problematic session, including thread
timelines, counter values, and budget states.

\subsection{GPU Timing and Cross-Domain
Correlation}\label{gpu-timing-and-cross-domain-correlation}

CPU instrumentation alone is insufficient for a rendering engine. The
GPU executes work asynchronously: a \texttt{queue.submit()} call returns
immediately, and the actual GPU execution happens later, potentially
overlapping with CPU work for the next frame. A frame-time regression
may be caused entirely by GPU work, but if the instrumentation system
only measures CPU time, the regression appears as ``nothing changed'' in
the profiler.

Smallworld's render thread maintains a \texttt{GpuTimingFeedback} ring:
a set of timestamp queries inserted before and after each render pass,
with results read back two or three frames later (GPU readback latency
is inherent to the pipelined model). Each timing result is stamped with
its frame index, allowing the instrumentation system to correlate GPU
pass durations with the CPU frame that generated them. The same timing
data feeds Tracy's GPU context, so the CPU timeline and the GPU timeline
appear on the same view, aligned by frame. A developer can see that the
``shadow'' pass on the GPU took 3.2 ms on the same frame where the CPU's
``cull.shadow\_views'' scope took 0.8 ms, and can reason about whether
the bottleneck is CPU-side culling overhead or GPU-side shadow
rendering.

The timestamp query mechanism is capability-gated: not all GPUs support
timestamp queries (\texttt{Capabilities::timestamp\_query} is probed at
startup). When timestamps are unavailable, the GPU timing ring is not
created, and the instrumentation system reports no GPU data rather than
incorrect data. This is the correct behavior: no data is better than
misleading data, and the absence of GPU timings in the overlay is a
visible signal that the developer should use an external GPU profiler on
that platform.

Figure \ref{fig:instrumentation-sinks} shows the complete
instrumentation data flow, from engine and game code through the macro
API to the three sinks, with the \texttt{BudgetRegistry} feeding the
overlay's budget table.

\input{figures/instrumentation-sinks.tex}

\subsection{Frame and Resource Telemetry}\label{sec-telemetry}

The instrumentation API provides the mechanism. The standard counter set
provides the vocabulary: a fixed set of counters and gauges that every
development build publishes, so that tooling (the overlay, CI regression
detection, and Tracy presets) can rely on their existence.
...
\tightlist
\item
  \textbf{Draw calls and instances per view.} The primary measure of
  rendering throughput. A regression in draw-call count often indicates
  a culling failure (entities that should be culled are being drawn) or
  a batching regression (draws that should be merged are being submitted
  separately).
\item
  \textbf{Culled counts per view.} The number of entities rejected by
  frustum culling, occlusion culling, and distance culling for each
  view. These mirror the \texttt{OcclusionFeedback} advisory data that
  the render thread sends back to the game thread, but they appear in
  the instrumentation overlay for immediate developer visibility.
\item
  \textbf{Brick residency.} For the Voxel Plugin: resident bricks,
  pinned bricks, and bricks evicted this frame. These counters are the
  direct readout of the streaming system's budget behavior for
  volumetric data.
\item
  \textbf{Streaming queue depth and in-flight bytes.} How many streaming
  requests are pending, and how many bytes are currently in the
  IO-to-staging-to-GPU pipeline. A steadily growing queue depth
  indicates that the streaming system's demand exceeds its IO or upload
  budget.
\item
  \textbf{Staging in-flight regions.} The number of staging-pool regions
  that have been populated by decode threads but not yet consumed by the
  render thread's GPU copy commands. This counter is the direct measure
  of the staging pool's utilization and its exposure to
  fence-reclamation delays.
\item
  \textbf{TLAS instance count.} When ray tracing is active, the number
  of instances in the top-level acceleration structure. A regression
  here may indicate that entities are being added to the TLAS that
  should be culled, increasing rebuild cost.
\end{itemize}

Games add their own counters through the same \texttt{counter!} and
\texttt{gauge!} macros. A game that tracks ``active AI agents'' or
``physics constraint count'' as custom counters gets those values in the
same overlay, on the same timeline, with the same CI regression
detection, without any engine modification.

\subsubsection{Designing for Diagnosis}\label{designing-for-diagnosis}

The instrumentation API and standard counters provide the data. But data
alone is not diagnosis. When a frame-time regression appears in a CI
trace, a developer must be able to trace it to a specific system, on a
specific thread, processing a specific set of entities, during a
specific phase of the frame. This requires three design properties that
the instrumentation system must support:

\textbf{Stable names.} Every scope, counter, and gauge has a string name
that is stable across builds. \texttt{"cull.shadow\_views"} is the same
scope in today's build and in last month's build. Automated regression
detection compares scope durations by name; if names change with
refactors, the comparison breaks and regressions go undetected. The
naming convention is hierarchical and dot-separated:
\texttt{"update.physics"}, \texttt{"extract.mesh\_delta"},
\texttt{"render.gbuffer"}. New scopes are added freely; existing scopes
are renamed only when the code they measure is itself renamed, and the
rename is noted in the commit message for CI tooling to handle.

\textbf{Scoped correlation.} When a scope nests inside another scope,
the instrumentation system records the parent-child relationship. This
produces a call tree, not a flat list. A developer seeing a 2 ms spike
in \texttt{"update"} can expand it to find that
\texttt{"update.physics"} took 1.8 ms, expand further to find that
\texttt{"update.physics.broad\_phase"} took 1.5 ms, and identify the
broad-phase collision detection as the bottleneck without reading any
code. The call tree is what makes scope-based profiling practical at
scale: without nesting, a flat list of hundreds of scopes is
unnavigable.

\textbf{Cross-thread identity.} When the game thread dispatches work to
the game worker pool, the worker's scopes must be attributable to the
dispatching scope. If the physics system dispatches island solves to
workers, the worker scopes should appear as children of
\texttt{"update.physics"} in the profiler, even though they execute on
different threads. Tracy supports this through its ``fiber'' and
``source location'' features; the engine's worker-pool integration
passes the parent scope's identity to child tasks so that the profiler
can reconstruct the logical call tree across thread boundaries.

These properties are not features of the instrumentation API itself;
they are design disciplines that the engine's use of the API must
follow. The API provides scopes, counters, and gauges. The engine's code
must use them consistently: stable names, proper nesting, and
cross-thread correlation. The result is an engine where every frame
produces a self-describing record of what happened, why, and how long
each part took, in enough detail to diagnose a performance regression
from a CI artifact without reproducing it locally.

\section{Reliability Is an Engine Capability}\label{sec-reliability}

The preceding sections established systems for naming resources,
limiting their consumption, and observing their behavior. These systems
are designed to be correct, but correctness is not self-maintaining.
Code changes. New subsystems are added. Existing contracts are extended.
A change that is correct in isolation may violate a cross-domain
invariant that was established three chapters ago. Without executable
verification, the engine's architectural contracts are prose that
developers must remember, not constraints that the build enforces.

Reliability in a game engine is not testing in the conventional
software-engineering sense of ``write unit tests for your functions.''
It is a layered verification strategy that targets different classes of
failure at different architectural boundaries. A unit test that verifies
a quaternion multiplication is valuable but narrow: it catches
arithmetic bugs in a single function. An integration test that verifies
the extract step produces correct render descriptions from a known world
state catches contract violations at the game-render boundary, a class
of bug that no unit test can reach. A scenario test that replays a
recorded input sequence through the full engine loop and checks that the
simulation produces the same world state catches emergent failures: bugs
that appear only when physics, streaming, rendering, and input interact
over hundreds of frames.

The game industry's relationship with testing has historically been
complicated. Studios often rely heavily on manual QA, with testers
playing the game to find bugs. Manual QA is irreplaceable for subjective
quality (does the game feel good? is the difficulty curve right? are the
textures readable at 4K?), but it is inefficient for regression
detection. A tester cannot re-play every possible input sequence after
every commit. Automated testing fills this gap, but adopting it in a
game engine context requires adapting conventional testing practices to
the engine's specific constraints: non-determinism from floating-point
arithmetic, frame-rate dependence, GPU-specific behavior, and the
difficulty of defining ``correct output'' for a visual medium.

Studios that have invested in automated testing report substantial
returns. Bungie has described using recorded input sessions to
regression-test \emph{Halo}'s gameplay systems across builds. Naughty
Dog's presentations on \emph{The Last of Us Part II} testing
infrastructure describe deterministic replay systems used to verify
gameplay correctness. Epic Games' testing framework for Unreal Engine
includes automated functional tests that run gameplay scenarios and
verify outcomes. The common thread is that effective game-engine testing
requires infrastructure, specifically the deterministic replay and
fixed-timestep foundations described in Chapters 4 and 5, applied to
verification.

\subsection{Tests at Architectural Boundaries}\label{sec-tests}

Smallworld's testing strategy is organized by the architectural boundary
each test targets, not by the conventional unit/integration/end-to-end
taxonomy. The boundaries in the engine are the load-bearing contracts:
the game-render firewall, the thread-ownership rules, the budget
invariants, the streaming residency guarantees, and the frame
lifecycle's phase ordering. Each boundary has failure modes that tests
at that boundary can detect, and that tests at other boundaries cannot.

\textbf{Component and math tests} verify the correctness of individual
data transformations: quaternion operations, frustum plane extraction,
AABB intersection, budget arithmetic, and generation-counter logic.
These tests are conventional unit tests: pure functions with known
inputs and expected outputs, no engine state, no threads, no GPU. They
run in milliseconds and catch arithmetic errors, off-by-one mistakes,
and edge-case handling failures. Every engine has these; they are
necessary but insufficient.

\textbf{Contract tests} verify that the interfaces between domains
produce correct results when composed. A contract test for the extract
step creates a \texttt{World} with known entities, runs the extract
function, and verifies that the resulting \texttt{FramePacket} contains
the expected deltas. A contract test for the budget system allocates
until the budget is exhausted and verifies that the pool's arbitration
response (evict, lower LOD, or reject) matches the configured policy. A
contract test for the handle system allocates, frees, reallocates, and
verifies that stale handles produce \texttt{None} rather than the new
occupant's data.

Contract tests are the most architecturally valuable category, because
they verify the engine's cross-domain invariants. A unit test for the
slot map verifies that the data structure works. A contract test for
entity despawn verifies that despawning an entity, which involves the
World, the ChangeTracker, the extract step, the render scene, and the
GPU resource pools, produces the correct downstream effects across all
participating systems. These tests catch the bugs that slip between
domains: a change to the despawn order that is correct from the World's
perspective but violates the extract step's assumption that physics
bodies are cleaned up before component removal.

\textbf{Scenario tests} exercise the full engine loop with recorded
inputs and deterministic replay. A scenario test loads a known scene,
feeds a recorded input sequence (the \texttt{Input} snapshots described
in Chapter 5), runs the engine for a fixed number of frames with a fixed
timestep, and verifies properties of the resulting world state: entity
positions, resource residency, budget pressure, counter values. Because
the timestep is fixed and the input is recorded, the simulation is
deterministic (subject to the caveats about floating-point
non-associativity discussed in Chapter 4), and the test produces the
same result on every run.

Scenario tests catch the emergent failures that no other test category
can reach. A streaming regression where the camera movement pattern
causes the streaming coordinator to thrash between two LOD levels,
consuming upload bandwidth without making progress. A physics
instability where a specific arrangement of rigid bodies causes the
solver to diverge after 300 frames. An input-handling bug where the
intent pattern drops a jump request during a specific
frame-rate/timestep combination. These failures emerge from the
interaction of multiple systems over time, and they require the full
engine loop, with all its phases and cross-domain communication, to
reproduce.

Figure \ref{fig:test-boundary-layers} illustrates the three test layers
and the failure classes each targets.

\input{figures/test-boundary-layers.tex}

\textbf{Fixed seeds, controlled clocks, and recorded inputs} are the
infrastructure that makes scenario tests reproducible. The engine's
random number generators accept explicit seeds, so procedural content
generation produces the same output on every run. The time source is
injectable: scenario tests provide a synthetic clock that advances by
exactly \texttt{fixed\_dt} per tick, eliminating variation from
wall-clock timing. Input is replayed from recordings, as described in
Chapter 5. Together, these controls eliminate the three primary sources
of non-determinism in gameplay: randomness, timing, and input. The
remaining source, floating-point non-associativity in parallel
reductions, is mitigated by ensuring that parallel work is partitioned
deterministically (same partition boundaries, same reduction order) and
by using comparison tolerances in scenario-test assertions.

\subsubsection{Property Tests}\label{property-tests}

Some engine invariants are best expressed as properties that must hold
for all valid inputs, rather than as specific input-output pairs.
\textbf{Property-based testing} (also called generative testing or
fuzzing) generates random inputs within specified bounds and verifies
that the invariant holds for each. The test framework generates
thousands of cases, and if any case fails, it reports the minimal
failing input.
...
\tightlist
\item
  \textbf{Handle invariants.} For any sequence of \texttt{insert},
  \texttt{remove}, and \texttt{get} operations on a slot map, a
  \texttt{get} with a stale handle must never return another occupant's
  data. The property test generates random operation sequences and
  verifies the invariant exhaustively.
\item
  \textbf{Budget accounting.} For any sequence of \texttt{allocate} and
  \texttt{free} operations, \texttt{used\_bytes} must never exceed
  \texttt{limit\_bytes} after a successful \texttt{can\_allocate} check,
  and must never go negative after a \texttt{free}.
\item
  \textbf{Serialization round-trips.} For any valid component value,
  \texttt{deserialize(serialize(value))\ ==\ value}. This catches
  encoding bugs, precision loss, and missing fields that specific-case
  tests might miss.
\end{itemize}

\subsubsection{Configuration, Features, and
Delivery}\label{sec-configuration}

An engine's reliability extends beyond runtime correctness into the
infrastructure that delivers it to players. Configuration management,
feature policy, continuous integration, and cross-platform packaging are
each potential failure points, and each deserves the same architectural
attention as the runtime systems they support.

\textbf{Configuration is validated data.} Engine and game configuration
(window size, audio quality, rendering tier, budget limits, keybindings)
is not a bag of key-value pairs. It is a typed, versioned data structure
with explicit defaults, range constraints, and an owner. Each
configuration domain (video settings, audio settings, input bindings,
engine internals) has a schema that defines the valid values, and the
engine validates configuration against the schema at load time. Invalid
values produce a diagnostic (logged through the instrumentation system)
and fall back to defaults, never to undefined behavior. The schema is
also what the settings UI reads: it knows the valid range for ``render
scale'' because the schema declares it, not because a programmer
hardcoded it in a slider widget.

Configuration versioning matters for long-lived games. When a new engine
version changes the default shadow resolution from 2048 to 4096,
existing user configurations that specify 2048 should be respected (the
user made a deliberate choice), but configurations that relied on the
old default should pick up the new one. The engine handles this by
versioning the configuration schema: a user's saved configuration
records the schema version it was written against, and a migration step
runs at load time to handle schema changes. Migrations are small,
testable functions, and they are tested in CI alongside the schema they
migrate.

\textbf{Feature policy} determines which engine capabilities are
available in a given build. A shipping build excludes the \texttt{egui}
overlay, the Tracy connection, and the chrome-trace export (the
zero-cost macros compile to no-ops). A development build includes all
three. A playtest build may include the overlay but not Tracy. These
policies are compile-time feature flags (Rust's \texttt{cfg} features),
not runtime toggles: the dead code is eliminated entirely, ensuring that
shipping builds pay zero cost for development instrumentation. The
feature matrix is tested in CI: the build system compiles the engine in
each supported configuration (shipping, development, playtest) on each
target platform, and each configuration must compile cleanly and pass
its applicable test suite.

\textbf{Continuous integration} runs the full test suite, compiles all
feature configurations, and archives performance traces for regression
detection. The CI pipeline is the engine's last line of defense before a
change reaches the team or the player. A CI run that takes ten minutes
catches most regressions within the PR review cycle. A CI run that takes
two hours catches regressions before the next playtest. The tradeoff is
wall-clock time versus coverage; the engine's test suite should be
structured so that the fast tests (component tests, contract tests,
compilation checks) run first and fail fast, while the slow tests
(scenario tests, cross-platform compilation) run in parallel and fail
with detailed diagnostics.

\textbf{Packaging and delivery} close the loop. A shipping build is a
cooked asset package, a compiled engine binary with shipping feature
flags, and a platform-specific installer or archive. The build system
verifies that the cooked assets match the engine's expected formats,
that no development-only assets leak into the package, and that the
binary size is within the target platform's constraints. For console
platforms, the build system additionally checks compliance with the
platform holder's packaging requirements (file layout, icon
specifications, content ratings metadata). These checks are automated: a
developer should not need to remember that PlayStation requires a
specific directory structure for DLC content. The build system knows,
because the packaging specification is part of the engine's
configuration.
...
This chapter established four infrastructure systems that the rest of
the engine depends on:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Identity.} Generational handles decouple resource identity
  from physical storage. Slot-map arenas provide O(1) access with
  use-after-free detection through generation counters. The type system
  prevents cross-type handle confusion at compile time, and the
  thread-ownership model prevents concurrent slot-map access
  structurally. Handles are cheap to copy, safe to serialize, and safe
  to hold across frames and thread boundaries.
\item
  \textbf{Budgets.} The \texttt{MemoryBudget} struct gives every
  resource pool a named limit, current usage, and high-water mark.
  Arbitration responses (evict LRU, lower LOD, reject) transform silent
  resource exhaustion into explicit, logged decisions. The
  \texttt{BudgetRegistry} ensures that every budget publishes its state
  as an instrumentation gauge, closing the loop between allocation and
  observability. Budget configuration is data, not code, enabling
  per-platform and per-hardware-tier profiles.
\item
  \textbf{Instrumentation.} Three macros (\texttt{profile\_scope!},
  \texttt{counter!}, \texttt{gauge!}) annotate engine and game code with
  zero shipping-build cost. Data flows to Tracy (deep analysis), the
  egui overlay (live monitoring with a budget table), and chrome-trace
  export (CI regression detection). GPU timing feedback unifies CPU and
  GPU timelines. The standard counter set provides a stable vocabulary
  for automated performance comparison across builds.
\item
  \textbf{Reliability.} Tests target architectural boundaries: component
  tests for pure logic, contract tests for cross-domain invariants, and
  scenario tests for emergent failures under deterministic replay.
  Property tests verify handle and budget invariants exhaustively.
  Configuration is validated, versioned, and migrated. Feature policy is
  compile-time. CI compiles all configurations, runs all test layers,
  and archives performance traces.
\end{enumerate}

The next chapter turns to the \textbf{resource pipeline and virtual
filesystem}: how assets travel from authored source content through
import, cooking, and the virtual filesystem to become runtime-ready
resources identified by the handles and managed by the budgets
established here.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Explain why a generational index prevents the ABA problem for resource
  handles. Describe the specific sequence of events (allocate, free,
  reallocate, stale access) and what each participant observes.
\item
  A \texttt{MemoryBudget} reports \texttt{pressure()\ ==\ 0.95} for the
  texture pool. The streaming system wants to upload a new texture that
  would push usage over the limit. Describe the arbitration sequence:
  what decisions does the pool make, in what order, and what happens if
  each response is insufficient?
\item
  Why does the architecture specify that ``zero references makes an
  asset evictable, not evicted''? Describe a gameplay scenario where
  eagerly freeing zero-reference assets would cause a visible problem
  for the player.
\item
  The instrumentation macros compile to no-ops in shipping builds. What
  would be the consequences of leaving them active? Consider CPU
  overhead, memory overhead, and security surface.
\item
  Explain the difference between a counter and a gauge in the
  instrumentation system. Give an example of a metric that is correctly
  modeled as a counter and one that is correctly modeled as a gauge, and
  explain why the other type would be wrong for each.
\item
  A scenario test replays a recorded input sequence and compares the
  resulting world state to a reference. The test passes on the
  developer's machine but fails on the CI server. List three possible
  sources of non-determinism that could cause this failure, and describe
  how the engine's infrastructure mitigates each.
\item
  The \texttt{BudgetRegistry} ensures that every \texttt{MemoryBudget}
  automatically publishes its state as instrumentation gauges. Why is
  this structural guarantee (enforced by the constructor API) preferable
  to a convention (documented in a coding standard)?
\item
  Describe a failure mode that a contract test can catch but a unit test
  cannot, and a failure mode that a scenario test can catch but a
  contract test cannot. Be specific about which systems interact and how
  the failure manifests.
\item
  The chapter states that Rust eliminates low-level memory-safety bugs
  but not capacity exhaustion. Explain the practical difference: what
  class of bugs does the borrow checker prevent, and what class requires
  the \texttt{MemoryBudget} infrastructure regardless of language?
\item
  A game ships on a console with 8 GB of total memory, of which the OS
  reserves 2 GB. The engine must distribute the remaining 6 GB across
  GPU pools (meshes, textures, render targets), CPU pools (asset cache,
  streaming buffers), and a staging pool. Describe the factors that
  determine the budget for each pool, and how the engine should respond
  when total demand exceeds 6 GB.
\end{enumerate}
...
\chapter{Replay, Recording, and
Determinism}\label{replay-recording-and-determinism}

\begin{quote}
\textbf{Draft chapter map.} A dedicated treatment of replay as a
cross-cutting engine capability, covering input replay, gameplay state
recording, network demo recording, determinism requirements, rollback
netcode, and automated testing.
\end{quote}

\section{Input Replay}\label{input-replay}

\begin{itemize}
\tightlist
\item
  The \texttt{Input} snapshot (Chapter 5) is a serializable,
  self-contained value type. Recording input means saving one snapshot
  per frame; replaying means feeding recorded snapshots into the INPUT
  phase in place of the live quarantine drain.
\item
  Recording format: header (engine version, \texttt{fixed\_dt},
  configuration hash) followed by a sequence of
  \texttt{(frame\_index,\ Input)} pairs. Only simulation-affecting
  fields need recording; window state and platform metadata can be
  omitted.
\item
  This is the simplest form of replay and the foundation for all others.
  \emph{StarCraft} (1998) used input-only replay because the game state
  was too large to snapshot at useful frequency; the replay file was
  under 100 KB for a 30-minute match.
\end{itemize}

\section{Gameplay State Replay}\label{gameplay-state-replay}

\begin{itemize}
\tightlist
\item
  UE5's Replay System records gameplay state (network-level demo
  recording), not raw input. It captures actor state, property
  replication, and RPC events as a network stream written to disk.
  Playback reconstructs the game by replaying the stream through the
  replication system.
\item
  This approach does not require determinism: the recording includes
  enough state to reconstruct the world regardless of floating-point
  drift or hash-map ordering differences. The trade-off is file size and
  the inability to ``what-if'' with modified input.
\item
  Compare the two approaches: input replay is compact and supports
  counterfactual replay but demands strict determinism. State replay is
  larger and cannot be modified but tolerates nondeterminism.
\end{itemize}

\section{Network Replay and Demo
Recording}\label{network-replay-and-demo-recording}

\begin{itemize}
\tightlist
\item
  Server-authoritative recording: the server records each client's input
  and the authoritative simulation state. Playback can reproduce the
  match from the server's perspective.
\item
  Client-side recording: the client records its local prediction and
  correction stream. Useful for bug reproduction but not authoritative.
\item
  Source engine and Unreal both use this approach for competitive match
  review and broadcast.
\end{itemize}

\section{Determinism Requirements and
Pitfalls}\label{determinism-requirements-and-pitfalls}

\begin{itemize}
\tightlist
\item
  Input replay only works if the simulation is deterministic given the
  same inputs. Chapter 4's fixed timestep provides half of this
  guarantee; the other half must come from the game and engine logic.
\item
  Sources of nondeterminism: uninitialized memory, hash-map iteration
  order (Rust's \texttt{HashMap} is intentionally randomized),
  floating-point non-associativity in parallel reductions, thread
  scheduling, system clock reads, random number generators without fixed
  seeds.
\item
  Mitigation strategies: deterministic hash maps (\texttt{IndexMap},
  \texttt{BTreeMap}), fixed seeds for RNG, sequential reductions for
  floating-point sums, avoiding system clock reads in simulation logic.
\item
  Testing determinism: run the same input recording twice and diff the
  world state at each frame. Any divergence indicates a nondeterminism
  source.
\end{itemize}

\section{Rollback Netcode}\label{rollback-netcode}

\begin{itemize}
\tightlist
\item
  GGPO (now integrated into virtually every modern fighting game)
  records each player's input per frame and replays the simulation to
  reconcile prediction errors. When the server receives a corrected
  input, it rolls back to the last confirmed state and re-simulates
  forward using the corrected input stream.
\item
  This is input replay running backward: the engine must be able to
  snapshot game state, restore it, and re-simulate a range of frames
  with different input.
\item
  Requirements: deterministic simulation, fast state snapshot/restore,
  and the ability to re-run N fixed ticks without rendering.
\item
  Racing games use input recording for ghost replay: \emph{Gran Turismo}
  and \emph{Forza} record the player's input (steering, throttle, brake)
  per tick and replay it to produce the ghost car in time trials.
\end{itemize}

\section{Automated Testing via
Replay}\label{automated-testing-via-replay}

\begin{itemize}
\tightlist
\item
  Recorded input sessions can be used for regression testing: a recorded
  playthrough that completes successfully on one build but fails on
  another isolates the breaking change to the commits between them.
\item
  Bungie has described using this approach for \emph{Halo}'s gameplay
  systems.
\item
  Property-based testing: generate random input sequences, run them, and
  check invariants (no crashes, no NaN positions, no physics tunneling).
\item
  Performance profiling: replay a demanding input sequence under a
  profiler to measure frame time regression without human variance.
\end{itemize}

\section{Engine vs.~Game
Responsibility}\label{engine-vs.-game-responsibility}

\begin{itemize}
\tightlist
\item
  \textbf{Research finding:} Unity is the only major engine that ships
  raw input recording as engine infrastructure (\texttt{InputEventTrace}
  and \texttt{InputRecorder} in the Input System package). UE5, Godot,
  and all other major engines treat replay as game code or plugin
  territory.
\item
  The engine's responsibility is to provide the \emph{foundation}:
  serializable input snapshots, deterministic fixed timestep, and (for
  rollback) fast state snapshot/restore. The recording infrastructure
  itself can live at either level.
\item
  Design question: should Smallworld ship a built-in recorder (like
  Unity) or provide only the foundation (like UE5/Godot)?
\end{itemize}

\part{Player Experience and Production}

\chapter{Developer Tools and
Diagnostics}\label{developer-tools-and-diagnostics}

\begin{quote}
\textbf{Draft chapter map.} Treat inspection, profiling, debugging
overlays, and editor infrastructure as production systems that the
engine provides, not afterthoughts bolted on at the end.
\end{quote}
...
\item
  ...
  requirements for a usable engine, not luxuries.
\item
  Connect tools to the observability architecture from Chapter 6 (named
  budgets, structured diagnostics, frame telemetry).
\item
  A game engine without tools is a library. The tools are what make it
  an engine.
\end{itemize}

\subsection{Debug Overlays and Diagnostic
Visualization}\label{debug-overlays-and-diagnostic-visualization}

\begin{itemize}
\tightlist
\item
  Frame-time graphs, GPU budget bars, streaming-state indicators,
  physics debug draw, audio source visualization.
\item
  These run inside the engine's render pipeline and read the same
  telemetry data that the budget system produces.
\item
  Overlay rendering as an engine service: game code can register custom
  debug draws without owning the render path.
\end{itemize}

\subsection{Profiling and
Instrumentation}\label{profiling-and-instrumentation}

\begin{itemize}
\tightlist
\item
  CPU timings, GPU timings, allocation pressure, queue depth, streaming
  state.
\item
  Distinguish current measurements from delayed or advisory feedback.
\item
  Connection to Chapter 6's instrumentation architecture: stable names,
  scopes, and correlation IDs make cross-thread work understandable.
\end{itemize}
...
\tightlist
\item
  Compare rapid diagnostic interfaces (immediate-mode debug UI for
  in-game inspection) with persistent editor models (retained-mode
  property editors, scene graphs).
\item
  Frame the choice around workflow and state ownership rather than
  fashion.
\item
  Immediate-mode is right for transient diagnostic panels. Retained-mode
  is right for editor state that persists across frames.
\end{itemize}

\subsection{Building an Editor as an Engine
...
\item
  ...
  editor.
\item
  The editor is an application built on the engine, not a separate
  architecture. It uses GameContext, the asset system, and the render
  pipeline like any game would.
\item
  This avoids a divergent tool architecture where the editor can do
  things the game cannot (or vice versa).
\end{itemize}

\section{Extensibility and AI-Assisted
Development}\label{extensibility-and-ai-assisted-development}
...
\item
  ...
  tractable.
\item
  A durable engine is an understandable engine: revisit the book's
  thesis that proven ideas survive, accidental legacy gets discarded,
  and modern tools are chosen deliberately.
\end{itemize}

\chapter{User Interface}\label{user-interface}

\begin{quote}
\textbf{Draft chapter map.} Cover UI as an engine subsystem that serves
both developer tools and player-facing game screens, with layout,
rendering, input integration, and the connection to accessibility.
\end{quote}
...
\tightlist
\item
  UI needs stable access to input, time, assets, and game-visible state
  without owning engine internals.
\item
  The same UI framework serves two audiences: developer tools (property
  inspectors, debug panels, editor viewports) and player-facing game UI
  (HUD, menus, inventory, dialogue).
\item
  The distinction is in content and styling, not in architecture. Both
  use the same layout engine, the same input path, and the same
  rendering contract.
\end{itemize}

\subsection{Layout and Composition}\label{layout-and-composition}

\begin{itemize}
\tightlist
\item
  Widget trees, layout algorithms (flexbox-style or constraint-based),
  and the data flow from game state to visual presentation.
\item
  UI state lives in the game's World or in dedicated UI state structs,
  not inside the renderer.
\item
  The UI framework reads game state and produces a widget tree. The
  renderer draws it. The separation mirrors the Game-Render Firewall.
\end{itemize}

\subsection{Input and Focus}\label{input-and-focus}

\begin{itemize}
\tightlist
\item
  Connection to Chapter 5's context stack: UI captures input by pushing
  a blocking action map.
\item
  Text entry routes through the IME path (Chapter 5's separate
  text-event pipeline).
\item
  Focus management, tab order, and keyboard navigation as structural
  features, not afterthoughts.
\end{itemize}

\subsection{Composition into the Presented
Image}\label{composition-into-the-presented-image}

\begin{itemize}
\tightlist
\item
  UI's render-facing contract: the UI layer produces draw commands that
  the renderer composites after (or during) post-processing.
\item
  Resolution independence: UI renders at native resolution even when the
  3D scene uses dynamic resolution scaling.
\item
  Menus and loading screens as ordinary lifecycle participants, not
  special cases.
\end{itemize}
...
\item
  Reproducible build inputs and release checks connect to the
  reliability practices from Chapter 6.
\item
  A release is an evidence-backed artifact, not merely a compiled
  executable.
\end{itemize}
...
\tightlist
\item
  A stable engine-facing contract must still be verified against real
  windowing, driver, input, and deployment environments.
\item
  Platform-specific UI conventions (safe areas on consoles, notch
  avoidance on mobile, DPI scaling on desktop) are handled through the
  same WindowState that Chapter 5 introduced.
\end{itemize}

\chapter{Accessibility}\label{accessibility}

\begin{quote}
\textbf{Draft chapter map.} A dedicated treatment of accessibility as a
cross-cutting engine capability, covering motor, visual, auditory, and
cognitive accommodations, the legal and certification landscape, and how
the engine provides primitives that games compose into accessibility
features.
\end{quote}

\section{Accessibility Is an Engineering
Discipline}\label{accessibility-is-an-engineering-discipline}

\begin{itemize}
\tightlist
\item
  Accessibility is not a feature list bolted on after the game is
  finished. It is a set of engineering constraints that, like
  performance or security, must be considered throughout the
  architecture.
\item
  The shift from afterthought to first-class concern: \emph{The Last of
  Us Part II} (2020) shipped over 60 accessibility options.
  \emph{Celeste} (2018) shipped an assist mode without compromising its
  identity. The Game Awards introduced a Best Accessibility category in
  2020.
\item
  The business case: a broader player base, positive press, awards
  recognition, and legal compliance.
\end{itemize}

\section{The Legal and Certification
Landscape}\label{the-legal-and-certification-landscape}

\begin{itemize}
\tightlist
\item
  \textbf{CVAA (21st Century Communications and Video Accessibility
  Act):} US law, enforcement began for games in 2019. Requires
  accessible input options for games with communication features.
\item
  \textbf{Xbox Accessibility Guidelines (XAGs):} Influence certification
  for Xbox and Game Pass titles. Explicitly recommend full input
  remapping, text-to-speech, subtitle customization, and high-contrast
  modes.
\item
  \textbf{Sony PlayStation accessibility requirements:} Similar
  provisions for PlayStation certification.
\item
  \textbf{EU European Accessibility Act (EAA):} Coming into effect, will
  affect games sold in EU markets.
\item
  Certification is not optional: a game that fails accessibility
  requirements may not ship on major platforms.
\end{itemize}

\section{Motor Accessibility}\label{motor-accessibility}

\begin{itemize}
\tightlist
\item
  \textbf{Input remapping} (covered in Chapter 5's action system): full
  rebinding, device-agnostic actions, persistent user overrides.
\item
  \textbf{Hold-to-toggle conversion:} ``Hold to aim'' becomes ``press
  once to aim, press again to stop.'' Engine primitive: a binding
  modifier that converts held state to toggled state.
\item
  \textbf{Sensitivity curves:} Per-axis adjustable curves for stick and
  mouse input. Engine primitive: a curve function applied in the binding
  evaluation pipeline.
\item
  \textbf{One-handed control presets:} Remap an entire half of the
  controller. Engine primitive: preset action map overlays loaded from
  \texttt{user://}.
\item
  \textbf{Auto-aim and aim assist:} Varying levels of targeting
  assistance. Typically game code, but the engine can provide aim-assist
  primitives (target queries, magnetism curves).
\item
  \textbf{Adaptive controllers:} Microsoft's Xbox Adaptive Controller
  and similar devices. The engine's \texttt{DeviceFilter} and
  \texttt{ControllerId} system handles these as standard HID devices
  with custom bindings.
\end{itemize}

\section{Visual Accessibility}\label{visual-accessibility}

\begin{itemize}
\tightlist
\item
  \textbf{Colorblind modes:} Protanopia, deuteranopia, tritanopia
  simulation and correction. Engine-level post-process filter or UI
  color palette swaps.
\item
  \textbf{High-contrast modes:} Reduce visual complexity, increase UI
  element contrast. Engine provides a UI theming system that games can
  override.
\item
  \textbf{Font scaling and readability:} Adjustable font sizes,
  dyslexia-friendly font options. Engine provides a text rendering
  system with scalable fonts.
\item
  \textbf{Subtitle customization:} Size, color, background opacity,
  speaker identification. Engine provides subtitle rendering primitives
  in the UI system.
\item
  \textbf{Reduced motion:} Disable or reduce screen shake, camera bob,
  particle intensity. Engine provides a motion-intensity scalar that
  systems can read.
\item
  \textbf{Screen magnification:} Zoom into portions of the screen.
  Engine-level render target scaling.
\end{itemize}

\section{Auditory Accessibility}\label{auditory-accessibility}

\begin{itemize}
\tightlist
\item
  \textbf{Subtitles and closed captions:} Not just dialogue, but sound
  effect descriptions (``{[}explosion nearby{]}'', ``{[}footsteps
  behind{]}'').
\item
  \textbf{Visual indicators for audio cues:} Directional threat
  indicators, visual beat cues in rhythm games. Game-level, but the
  engine provides the audio event system that triggers them.
\item
  \textbf{Mono audio and balance adjustment:} Engine-level audio mixing
  option for single-ear hearing.
\item
  \textbf{Volume sliders per category:} Music, SFX, dialogue, ambient.
  Standard engine audio architecture (Chapter 20).
\end{itemize}

\section{Cognitive Accessibility}\label{cognitive-accessibility}

\begin{itemize}
\tightlist
\item
  \textbf{Difficulty scaling:} Not just enemy health, but pacing, puzzle
  hints, navigation assistance.
\item
  \textbf{Objective reminders and waypoints:} Reduce memory load.
\item
  \textbf{Simplified UI modes:} Reduce information density.
\item
  \textbf{Tutorial replay and pause-anywhere:} Structural engine support
  (pause is already a time-scale-zero operation from Chapter 4).
\end{itemize}

\section{Engine Primitives vs.~Game
Policy}\label{engine-primitives-vs.-game-policy}

\begin{itemize}
\tightlist
\item
  The engine provides accessibility \emph{primitives}: binding
  modifiers, color palette systems, font scaling, audio mixing
  categories, motion-intensity scalars, subtitle rendering.
\item
  The game provides accessibility \emph{policy}: which options to
  expose, how to present them, what presets to offer, what the default
  experience looks like.
\item
  The settings screen is game code. The engine provides the data types
  and persistence path (\texttt{user://}). The game provides the UI,
  matching the game's visual style.
\item
  Design question: should the engine ship with a default accessibility
  settings panel that games can customize, or only provide primitives?
\end{itemize}
...
\tightlist
\item
  Automated testing: colorblind simulation screenshots, contrast ratio
  checks, font-size minimum verification.
\item
  Manual testing: involve players with disabilities in playtesting. The
  AbleGamers Charity and SpecialEffect provide consultation services.
\item
  Certification checklists: map engine primitives to XAG and CVAA
  requirements.
\end{itemize}

\section{Industry References}\label{industry-references}

\begin{itemize}
\tightlist
\item
  \emph{The Last of Us Part II} (Naughty Dog, 2020): 60+ accessibility
  options, industry benchmark.
\item
  \emph{Celeste} (Matt Makes Games, 2018): Assist mode in a challenging
  game.
\item
  \emph{Forza Horizon 5} (Playground Games, 2021): Sign language
  interpreter for cutscenes, screen reader support for menus.
\item
  \emph{God of War Ragnarok} (Santa Monica Studio, 2022): Over 70
...
\chapter{Game Production}\label{game-production}

\begin{quote}
\textbf{Draft chapter map.} Walks the reader through the production
process that turns engine capabilities into a shipped game. Covers the
full lifecycle from vision to post-release, with emphasis on the phases,
gates, and disciplines that keep a project on track.
\end{quote}

\section{The Production Lifecycle}\label{the-production-lifecycle}

\begin{itemize}
\tightlist
\item
  Game development follows a well-established lifecycle that the
  industry arrived at through decades of shipped titles: vision,
  pre-production, prototypes, vertical slice, full production, alpha,
  beta, release candidate, release, and post-release.
\item
  These are not dates on a schedule. Each phase answers a different
  question, and the gate between phases is a decision about whether the
  answer is good enough to proceed.
\item
  The lifecycle applies regardless of team size. A solo developer and a
  200-person studio go through the same phases, but the formality and
  documentation differ.
\end{itemize}

\subsection{Vision: ``What Game Are We
Making?''}\label{vision-what-game-are-we-making}

\begin{itemize}
\tightlist
\item
  Establish the small set of things that must remain true: game pillars,
  target audience, core loop, visual identity, world structure,
  progression philosophy, tone, and player fantasy.
\item
  The output is not a 500-page GDD. It is a compact body of canonical
  direction that later decisions can be evaluated against.
\item
  Gate: leave this phase when everyone can answer ``what is this game?''
  in approximately the same way.
\end{itemize}

\subsection{Pre-Production: ``Can This Game
Work?''}\label{pre-production-can-this-game-work}

\begin{itemize}
\tightlist
\item
  Attack uncertainty, not content volume. Identify the risky questions
  and build prototypes specifically to answer them.
\item
  A prototype is cheap and disposable. Its purpose is proving a design
  or technical hypothesis, not becoming final production content. ``Does
  this work?'' is a different question from ``Can we ship this?''
\item
  Unity's best-practice guidance emphasizes prototyping as a way of
  testing ideas efficiently before committing to larger-scale creation.
\item
  Gate: leave pre-production when the major unanswered questions have
  become manageable risks rather than guesses.
\end{itemize}

\subsection{The Vertical Slice: ``Can We Actually Make
This?''}\label{the-vertical-slice-can-we-actually-make-this}

\begin{itemize}
\tightlist
\item
  A vertical slice is a production prototype: a small portion of the
  real game built through the complete production pipeline at
  approximately target quality.
\item
  It answers two questions simultaneously. The creative proof: ``Is this
  game good?'' The production proof: ``Can we make this game repeatedly
  at an affordable rate?''
\item
  The vertical slice is the most important phase for establishing
  production confidence, because it forces every discipline (art,
  design, engineering, audio, narrative, QA) to cooperate through the
  actual pipeline.
\item
  Gate: leave when the slice demonstrates both the experience and the
  process used to manufacture it.
\end{itemize}

\subsection{Full Production: ``Make the
Game''}\label{full-production-make-the-game}

\begin{itemize}
\tightlist
\item
  The longest phase. The project shifts from inventing the factory to
  running the factory.
\item
  Good production creates finished vertical chunks continuously rather
  than assembling disciplines sequentially. Each chunk crosses
  disciplines and gives real feedback early.
\item
  Asset and content pipelines matter here: a biome, a creature, a quest
  should each have a repeatable production pipeline from brief through
  integration to QA.
\end{itemize}

\subsection{The Work-Item Lifecycle}\label{the-work-item-lifecycle}

\begin{itemize}
\tightlist
\item
  Every individual piece of content follows the same flow: backlog,
  design, prototype/blockout, approval, production, integration, review,
  playtest/QA, polish, done.
\item
  ``Done'' means the player-facing experience works, not that one
  discipline finished their part.
\item
  Design review happens before expensive production. Changing a
  silhouette on a sketch costs nothing. Changing it after modeling, UVs,
  texturing, rigging, 20 animations, hitboxes, and VFX is painful. Stage
  gates prevent expensive downstream rework.
\end{itemize}

\subsection{Alpha, Beta, and Release}\label{alpha-beta-and-release}

\begin{itemize}
\tightlist
\item
  \textbf{Alpha:} every intended major system and the complete game
  progression exist and can be played end-to-end. Not polished, but
  complete. The question shifts from ``what should we add?'' to ``what
  prevents this from shipping?''
\item
  \textbf{Beta:} stabilization. Stop inventing, make it good. Bugs,
  crashes, performance, memory, balance, UX, accessibility,
  localization, hardware compatibility. Every change must make the
  existing game more shippable.
\item
  \textbf{Release candidate:} a build treated as if it were the released
  product. No random improvements enter the build. Protect stability.
\item
  \textbf{Release:} package the known-good RC. Preserve source revision,
  data revision, build configuration, toolchain version, and release
  artifacts for reproducibility.
\item
  \textbf{Post-release:} crash fixes, save corruption fixes,
  compatibility, performance patches, balance, quality-of-life. New
  content enters a new production cycle.
\end{itemize}

\section{Playtesting Runs Through the Entire
Process}\label{playtesting-runs-through-the-entire-process}

\begin{itemize}
\tightlist
\item
  Do not build the game first and test at the end. Design,
  implementation, playtest, and iteration form a continuous loop.
\item
  Important assumptions need empirical answers, not debates. These
  become playtest questions with measurable outcomes.
\end{itemize}

\section{Engine Team and Game Team Meet at a
Contract}\label{engine-team-and-game-team-meet-at-a-contract}

\begin{itemize}
\tightlist
\item
  The engine provides capabilities. The game production process turns
  design intent into finished, tested player experience.
\item
  The contract flows: game need, engine requirement, engine feature,
  engine tests, versioned capability, game uses it.
\item
  Engine builds capabilities. Game production builds experiences from
  those capabilities. Technology should not push features into the game
  merely because they are interesting to implement. The game pulls
  technology into existence.
\end{itemize}

\section{Production Scaling}\label{production-scaling}

\begin{itemize}
\tightlist
\item
  Do not scale production until the vertical slice has proven both the
  experience and the process used to manufacture it.
\item
  The first time through a content type (biome, creature, quest chain)
  is expensive because the process is being designed simultaneously. The
  second time is cheaper. The tenth time is routine.
\item
  Every content type should have a repeatable pipeline and a
  completeness contract (what must a biome satisfy? what must a creature
  deliver?).
\item
  80\% established vocabulary, 20\% new idea per content unit.
  Data-driven production can accidentally produce formulaic content
  without novelty.
\end{itemize}
