...
into engine types, and edges are computed by diffing the two frames.
...
\NormalTok{                input}\OperatorTok{.}\NormalTok{mouse}\OperatorTok{.}\NormalTok{delta }\OperatorTok{+=}\NormalTok{ d}\OperatorTok{;}       \CommentTok{// accumulate, never replace}
            \OperatorTok{\}}
...
enforce cursor capture. And \texttt{winit} knows nothing about
controllers; the engine pairs it with \texttt{gilrs} or a similar
...
the \texttt{Input} struct, the action system, the context stack, and
...
dimensions, screen info) and \texttt{Input} (device state). The
separation is not accidental: window state changes infrequently and has
lifecycle consequences (a resize triggers surface reconfiguration on the
GPU), while input state changes every frame and drives gameplay logic.
Collapsing them into a single structure would obscure this difference
and tempt game code to poll window dimensions at the same frequency as
key state, which is wasteful and conceptually wrong.
...
questions; it does not register callbacks) and frame-coherent (every
query within a tick returns values from the same snapshot).

Smallworld's raw polling API follows this consensus. The \texttt{Input}
struct is the engine's per-frame snapshot of all connected devices.
Listing~\ref{lst-input-struct} defines the struct and its query methods
for keyboard state, mouse state, and controller state.
...
three queries answer different temporal questions, and confusing them is
one of the most common input bugs in game development. A jump system
that reads ``is the spacebar down?'' instead of ``was the spacebar
pressed this frame?'' will apply the jump impulse on every frame the key
is held, launching the character into orbit. A weapon system that reads
``pressed'' when it should read ``held'' will fire exactly once and then
stop, even if the player is holding the trigger. These bugs are easy to
write and surprisingly hard to diagnose, because the system \emph{works}
in the common case (a quick tap) and fails only when timing matters.

The three queries answer distinct temporal questions:
...
\texttt{Completed} phases. The cost is one additional bit per key per
...
The accumulation model is particularly important for high-polling-rate
...
of a single per-frame value. This is why every engine that takes mouse
input seriously (and certainly every engine used for competitive
shooters) accumulates mouse deltas rather than sampling the latest
position.
...
from the controller's current state. But physical sticks are imperfect
mechanisms: they drift, they have manufacturing tolerances, and they
wear over time. The Nintendo Switch Joy-Con drift issue, which became so
widespread that it prompted a class-action lawsuit and a free repair
program, is the most visible example of a problem that every controller
exhibits to some degree. A brand-new Xbox controller may report
\texttt{(0.02,\ -0.01)} when the stick is centered. An older DualSense
may drift further. Without compensation, the game would register
constant low-magnitude movement even when the player is not touching the
stick. Dead zones (discussed in the action mapping section below)
address this at the binding level, but the raw polling API reports the
unfiltered value, leaving the choice of threshold to the consumer.
...
Local multiplayer has been part of gaming since two players shared an
arcade cabinet in \emph{Pong} (1972), and the conventions around
controller assignment have been remarkably stable. From the Super
Nintendo's four controller ports through the Xbox 360's wireless
controller pairing to the Nintendo Switch's Joy-Con handshake, the
standard is four simultaneous controllers with fixed slot assignment:
player one is controller zero, player two is controller one, and so on.
Some platforms support more (the Nintendo 64 had four ports; the Wii
supported eight Wii Remotes), but four is the common denominator that
...
\item
  \textbf{Plain value semantics.} The array is a plain value type with
  no references or heap indirection. It can be copied, sent across
  threads, and serialized without lifetime concerns.
\end{itemize}

Four slots accommodate the standard case. Games that need more
controllers (a party game with eight players, for example) would extend
the array or use a different backing store, but four is the pragmatic
default that matches the conventions of every major platform. The
fixed-size choice also means that the \texttt{Input} struct's memory
layout is constant and predictable, which matters for the serialization
story discussed in the replay section below: a fixed layout serializes
to a fixed-size record, simplifying the recording format.
...
can be migrated to \texttt{axis2("move")}, and it gains rebindability,
...
Anyone who has played a game where opening the inventory did not prevent
the character from moving, or where pressing Escape closed a dialog
\emph{and} opened the pause menu simultaneously, has experienced the
consequences of ad-hoc input routing.
...
\texttt{\_unhandled\_input()}. Each approach encodes the same
fundamental idea: input routing is a structured priority problem, not a
collection of per-system boolean checks.

Smallworld's version of this structure is a \textbf{context stack}.
Active action maps are pushed onto a stack; the topmost map receives
input first. Each map carries a \texttt{passthrough} flag that controls
...
The history of key rebinding in games traces a path from afterthought to
first-class feature. In the early 1990s, most games shipped with fixed
key layouts. Quake (1996) was among the first to offer deep rebinding
through its console (\texttt{bind\ W\ +forward}), but the system was
bolted onto a set of text commands and required the player to understand
the engine's action vocabulary. For years afterward, rebinding remained
inconsistent: some games offered full remapping, others allowed only
partial changes, and many console ports shipped on PC with no rebinding
at all. The industry treated rebinding as a convenience feature for
power users, not as a design requirement.

That changed as the player base diversified and accessibility became a
business and legal concern. The 21st Century Communications and Video
Accessibility Act (CVAA) in the United States, which began enforcement
for games in 2019, requires that games with communication features
provide accessible input options, including remapping. Microsoft's Xbox
Accessibility Guidelines (XAGs), which influence certification for Xbox
and Game Pass titles, explicitly recommend full input remapping. Sony's
accessibility requirements for PlayStation certification include similar
provisions. The Game Awards introduced a Best Accessibility category in
2020, and studios like Naughty Dog (\emph{The Last of Us Part II})
demonstrated that comprehensive accessibility, including remapping,
one-handed play modes, and custom control schemes, could ship in AAA
titles without compromising the core experience. Rebinding is no longer
a convenience; it is a certification prerequisite on major platforms and
a legal requirement in some jurisdictions.

The architectural consequence is that rebinding must be a data
operation, not a code change. Action maps are \textbf{assets}. They are
loaded through the asset pipeline, identified by name (e.g.,
\texttt{"gameplay"}, \texttt{"ui"}, \texttt{"vehicle"}), and their
bindings can be modified at runtime. This is the foundation of
player-facing rebinding: the settings screen reads the current action
map, displays the bindings, and lets the player modify them.
...
their defaults. The overlay model has a useful property for version
...
action) does not conflict. Both coexist without a migration step.
...
types that the player edits at runtime. ``Reset to defaults'' is a file
deletion: remove the user override, reload the default map from the
game's asset package, and rebuild the input state. The default map is
never modified; it is always available as the canonical fallback.
...
Section 5.1 established that the quarantine converts pushed OS events
into a polled snapshot. The distinction is worth revisiting briefly in
the context of frame timing, because the interaction between the
snapshot and the phase lifecycle has consequences that the quarantine
discussion alone does not cover.
...
delivered twice (a real problem on some platforms with auto-repeat). A
snapshot is also a compact, self-contained record: recording input for
...
(discussed in section 5.4), not through the action system.
...
at any time; the INPUT phase drains that buffer and freezes the
snapshot. The FIXED phase may read the snapshot (with caveats about
edges, discussed below). The UPDATE phase is the primary consumer for
input-driven gameplay logic. The LATE phase benefits from the
consistency guarantee but does not typically read input directly. There
is no second input poll between FIXED and UPDATE, or between UPDATE and
LATE. This is a consequence of the fixed-phase model described in
Chapter 4: phases execute in a strict, invariant order, and each phase
sees the world as its predecessor left it.
...
already manages which action map is active; cursor capture state follows
the same transitions, ensuring that the mouse behaves correctly for each
activity without ad-hoc management.
...
of motion events per 16 ms frame. Competitive FPS players are especially
sensitive to mouse input fidelity: the difference between a mouse that
reports at 125 Hz and one that reports at 1000 Hz is perceptible in fast
flick-shots, and any engine that drops motion events between polls will
feel less responsive than one that accumulates them. Without
accumulation, the engine would either lose events (if it reads only the
latest) or force systems to iterate over an event queue (which breaks
the polled-snapshot model). Accumulation is the approach that preserves
both the full range of player motion and the simplicity of the polled
snapshot.
...
This technique has a long and successful history in the game industry.
Real-time strategy games pioneered input-only replay out of necessity:
\emph{StarCraft} (1998) recorded only player commands (not world state),
because the game state was too large to snapshot at useful frequency.
The replay file was tiny (often under 100 KB for a 30-minute match), and
deterministic simulation reproduced the entire game from that record.
Fighting games adopted the same principle for a different reason:
rollback netcode, as implemented in GGPO (now integrated into virtually
every modern fighting game), records each player's input per frame and
replays the simulation to reconcile prediction errors. When the server
receives a corrected input, it rolls back to the last confirmed state
and re-simulates forward using the corrected input stream. This only
works if the simulation is deterministic given the same inputs. Racing
games use input recording for ghost replay: \emph{Gran Turismo} and
\emph{Forza} record the player's input (steering, throttle, brake) per
tick and replay it to produce the ghost car in time trials.
...
time. Bungie has described using recorded input sessions to
regression-test \emph{Halo}'s gameplay systems across builds: a recorded
...
Network prediction and rollback also rely on input recording: the server
records each client's input and replays the authoritative simulation.
These are advanced features, but they all begin with the same
foundation: a compact, per-frame input record and a deterministic
timestep.
...
\item
  \textbf{Platform conventions.} Different platforms have different
  button conventions for the same semantic action. These conventions are
...
conventions. A player who uses a one-handed controller, a player who
cannot reach certain keys due to a physical condition, a player who
prefers inverted Y-axis for historical reasons, and a player who simply
wants ESDF instead of WASD all benefit from the same system. \emph{The
Last of Us Part II} (2020) set a new industry benchmark by shipping over
60 accessibility options, including full remapping, button-hold toggles,
and custom control presets. \emph{Celeste} (2018) demonstrated that
accessibility features (an assist mode that let players customize game
speed and invincibility) could ship in a challenging game without
diminishing its identity. These examples shifted the industry
conversation from ``should we support accessibility?'' to ``how deeply
can we support it?''

The action system provides remapping as a structural consequence of its
...
being a generic engine dialog. It also means the engine does not impose
a specific accessibility model: the game can build toggle modes,
sensitivity curves, one-handed presets, and any other accommodation on
top of the same binding primitives that power standard remapping.
...
\subsection{The Platform Boundary}\label{the-platform-boundary}

The platform boundary is the narrow surface through which OS-specific
types enter the engine and are translated into engine-owned types. Every
engine draws this line somewhere, and the placement determines how much
game code must change when the engine ships on a new platform. Draw the
line too deep inside the engine, and platform details leak into gameplay
systems that should never have seen them. Draw it too wide (abstracting
windowing, file IO, threading, timers, and input behind a single
platform interface), and you build an abstraction layer that duplicates
work already done by the libraries beneath it. The sweet spot is a thin
adapter that translates exactly the types that are genuinely
platform-specific, and nothing more.

Unreal draws this line at its \texttt{FGenericPlatformMisc} and
\texttt{FGenericApplication} classes, which provide platform-specific
subclasses for each target. The depth is justified: Unreal ships on
platforms with proprietary input APIs that demand custom backends. Unity
draws the line at its native player loop, where platform-specific C++
code feeds events into the managed C\# runtime. Godot draws it at
\texttt{DisplayServer} and the OS-specific input handlers. In each case,
the boundary's purpose is the same: game code above the line never sees
a platform type.
...
\textbf{Thread-safe value types on all outputs.} The \texttt{Input}
struct, \texttt{WindowState}, and all types they contain are plain value
types with no references, no borrowed window handles, and no
platform-specific pointer types. They can be sent across threads, stored
in arenas, serialized, and recorded without lifetime concerns. In Rust,
this property is enforced by the
\texttt{Send\ +\ \textquotesingle{}static} bounds that the compiler
checks at every thread crossing. In C++, achieving the same guarantee
requires discipline and convention; Rust makes it a compile-time
invariant.
...
mutates), the engine routes them through separate paths on
\texttt{GameContext}. Stack operations go through a dedicated method on
\texttt{GameContext} rather than through the immutable \texttt{input}
borrow, ensuring that reading and writing never conflict. This
separation reflects a general API design principle: when a subsystem has
both read-only queries and mutating commands, exposing them through
separate access paths prevents callers from accidentally mixing the two.
